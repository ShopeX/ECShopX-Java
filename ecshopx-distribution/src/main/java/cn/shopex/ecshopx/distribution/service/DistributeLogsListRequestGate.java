...
@Service
public class DistributeLogsListRequestGate {
	...
	public DistributeLogsListRequestGate(
	...
	public Map<String, Object> validateBeforeList(HttpServletRequest request, Map<String, Object> merged) {
		...
		if (!"salesperson_workwechat".equals(source)) {
			distributorMenuPermissionService.assertRouteAllowed(
					user, DistributorMenuPermissionService.ROUTE_ALIAS_FRONT_WXAPP_DISTRIBUTION_LOG);
		}
	...
	public Map<String, Object> validateBeforeCompanyCount(HttpServletRequest request) {
		Object userRaw = request.getAttribute(OperatorJwtAuthenticationFilter.OPERATOR_JWT_USER_DATA);
		if (!(userRaw instanceof Map<?, ?> rawMap)) {
			throw new UnauthorizedException("未登录");
		}
		...
		if (!"salesperson_workwechat".equals(source)) {
			distributorMenuPermissionService.assertRouteAllowed(
					user, DistributorMenuPermissionService.ROUTE_ALIAS_FRONT_WXAPP_DISTRIBUTION_COUNT);
		}
	...
	private static void validatePagination(Map<String, Object> merged) {
