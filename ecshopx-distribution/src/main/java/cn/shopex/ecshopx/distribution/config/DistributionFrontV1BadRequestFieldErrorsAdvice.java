...
package cn.shopex.ecshopx.distribution.config;
...
/**
 * Dingo-style {@link BadRequestException} for distribution front v1: when the handler declares
 * {@code badRequest != NONE}, builds JSON with {@code data.message}, {@code data.status_code}, and
 * {@code data.errors} when field errors are present (global handler does not expand {@code errors}).
 */
@RestControllerAdvice(basePackages = "cn.shopex.ecshopx.distribution.api.front.v1")
@Order(Ordered.HIGHEST_PRECEDENCE)
public class DistributionFrontV1BadRequestFieldErrorsAdvice {
