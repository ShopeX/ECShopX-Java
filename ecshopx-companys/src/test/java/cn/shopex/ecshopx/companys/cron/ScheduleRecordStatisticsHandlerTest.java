...
class ScheduleRecordStatisticsHandlerTest {
	...
	@InjectMocks
	private ScheduleRecordStatisticsHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleRecordStatisticsHandler.class);
	...
	private static boolean matchesRecordStatisticsOptions(DispatchOptions opts) {
		return opts != null
	...
	@Test
	@DisplayName("success: two dispatchJob calls in order, no synchronous shopping guide, INFO with jobDispatches")
	void executeOk() {
		handler.execute();
		...
		inOrder
				...
				.dispatchJob(
						eq(CompanysBundleDispatchJobNames.RECORD_STATISTICS_JOB),
						argThat(m -> m != null && m.isEmpty()),
						argThat(ScheduleRecordStatisticsHandlerTest::matchesRecordStatisticsOptions));
		inOrder
				.verify(dispatchFacade)
				.dispatchJob(
						eq(CompanysBundleDispatchJobNames.SALESPERSON_RECORD_STATISTICS_JOB),
						argThat(m -> m != null && m.isEmpty()),
						argThat(ScheduleRecordStatisticsHandlerTest::matchesRecordStatisticsOptions));
		...
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								...
								&& e.getFormattedMessage().contains("record-statistics")
	...
	void executeFailOnFirstDispatch() {
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("record-statistics");
		assertThat(listAppender.list)
				.anyMatch(
	...
	@Test
	@DisplayName("failure on second dispatchJob: CronAlertEvent after first enqueue")
	void executeFailOnSecondDispatch() {
