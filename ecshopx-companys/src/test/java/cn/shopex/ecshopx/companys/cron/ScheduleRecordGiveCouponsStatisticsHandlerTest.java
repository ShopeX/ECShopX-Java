...
class ScheduleRecordGiveCouponsStatisticsHandlerTest {
	...
	@InjectMocks
	private ScheduleRecordGiveCouponsStatisticsHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleRecordGiveCouponsStatisticsHandler.class);
	...
	private static boolean matchesGiveCouponsCronDispatchOptions(DispatchOptions opts) {
		return opts != null
	...
	@Test
	@DisplayName("success: dispatch job 181 then 182 in order, INFO jobDispatches=2")
	void executeOk_dispatchesJob181Then182_inOrder_logsTwoDispatches() {
		handler.execute();
		InOrder inOrder = inOrder(dispatchFacade);
		inOrder
				.verify(dispatchFacade)
				.dispatchJob(
						eq(CompanysBundleDispatchJobNames.RECORD_STATISTICS_JOB_GIVE_COUPONS_SCHEDULE),
						argThat(m -> m != null && m.isEmpty()),
						argThat(ScheduleRecordGiveCouponsStatisticsHandlerTest::matchesGiveCouponsCronDispatchOptions));
		inOrder
				.verify(dispatchFacade)
				.dispatchJob(
						eq(CompanysBundleDispatchJobNames.SALESPERSON_GIVE_COUPONS_RECORD_STATISTICS_JOB),
						argThat(m -> m != null && m.isEmpty()),
						argThat(ScheduleRecordGiveCouponsStatisticsHandlerTest::matchesGiveCouponsCronDispatchOptions));
		...
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								...
								&& e.getFormattedMessage().contains("record-give-coupons-statistics")
	...
	void executeFail_onFirstDispatchJob_cronAlertAndRethrow() {
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("record-give-coupons-statistics");
		assertThat(listAppender.list)
				.anyMatch(
