...
@Service
public class FrontDepositPayEnabledPortImpl implements FrontDepositPayEnabledPort {

	private final RechargeSettingRedisService rechargeSettingRedisService;

	public FrontDepositPayEnabledPortImpl(RechargeSettingRedisService rechargeSettingRedisService) {
		this.rechargeSettingRedisService = rechargeSettingRedisService;
	}

	@Override
	public boolean isDepositPayEnabled(long companyId) {
		Map<String, Object> recharge = rechargeSettingRedisService.handle(companyId, Collections.emptyMap());
		return Boolean.TRUE.equals(recharge.get("recharge_status"));
	}
}
