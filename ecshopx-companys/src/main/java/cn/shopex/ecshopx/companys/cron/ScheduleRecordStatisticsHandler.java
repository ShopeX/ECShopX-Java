...
public class ScheduleRecordStatisticsHandler {

	private static final String SHORT = "record-statistics";
	...
	public void execute() {
		...
		try {
			...
			dispatchFacade.dispatchJob(
					CompanysBundleDispatchJobNames.RECORD_STATISTICS_JOB, new LinkedHashMap<>(), options);
			dispatchFacade.dispatchJob(
					CompanysBundleDispatchJobNames.SALESPERSON_RECORD_STATISTICS_JOB,
					new LinkedHashMap<>(),
					options);
			long costMs = System.currentTimeMillis() - start;
			log.info("[cron][record-statistics] done, costMs={}, jobDispatches=2", costMs);
		} catch (Exception e) {
			log.error(
					"[cron][record-statistics] failed, costMs={}",
