 ...
 */
@Data
@MpTable(value = "adapay_paymemt_confirm", comment = "adapay支付确认表", indexes = {@MpIndex(name = "idx_company_id", columns = {"company_id"}), @MpIndex(name = "idx_order_id", columns = {"order_id"}), @MpIndex(name = "idx_order_no", columns = {"order_no"}), @MpIndex(name = "distributor_id", columns = {"distributor_id"})})
public class AdapayPaymemtConfirm {
    ...
    @MpField(value = "payment_id", columnType = "string", nullable = true, comment = "Adapay生成的支付对象id 对应order表的transaction_id")
