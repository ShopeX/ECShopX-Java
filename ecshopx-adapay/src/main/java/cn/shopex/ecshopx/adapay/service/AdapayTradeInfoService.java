...
/**
 * Builds admin trade detail payloads including aggregate-derived state when rows exist.
 * <p>
 * When no aggregated payment rows exist for the order, the payload keeps the stored {@code trade_state} on the
 * trade row instead of deriving SUCCESS / partial / full refund from refunds.
 */
@Service
public class AdapayTradeInfoService {

	private final TradeMapper tradeMapper;
	private final AdapayTradeListForInfoQueryService adapayTradeListForInfoQueryService;
	private final MembersMapper membersMapper;
	private final MembersInfoMapper membersInfoMapper;
	private final AdapayTradeRefundSnippetQueryService refundSnippetQueryService;
	private final AdapayDivFeeInfoAssembler adapayDivFeeInfoAssembler;
	...
	public AdapayTradeInfoService(
			TradeMapper tradeMapper,
			AdapayTradeListForInfoQueryService adapayTradeListForInfoQueryService,
			MembersMapper membersMapper,
			MembersInfoMapper membersInfoMapper,
			AdapayTradeRefundSnippetQueryService refundSnippetQueryService,
			AdapayDivFeeInfoAssembler adapayDivFeeInfoAssembler,
		...
		this.adapayTradeListForInfoQueryService = adapayTradeListForInfoQueryService;
		this.membersMapper = membersMapper;
		this.membersInfoMapper = membersInfoMapper;
		this.refundSnippetQueryService = refundSnippetQueryService;
		this.adapayDivFeeInfoAssembler = adapayDivFeeInfoAssembler;
	...
	public Object getTradeInfo(String tradeId, Map<String, Object> jwtMap) {
		...
		List<Map<String, Object>> aggRows = adapayTradeListForInfoQueryService.listAggregatedForOrder(tradeCompanyId,
				trade.getOrderId());
		String tradeStateForPayload;
		if (aggRows.isEmpty()) {
			tradeStateForPayload = trade.getTradeState();
		} else {
			tradeStateForPayload = String.valueOf(aggRows.get(0).get("tradeState"));
		}
		Map<String, Object> out = tradeToSnakeMap(trade);
		out.put("trade_state", tradeStateForPayload);
		out.put("refund_list", refundSnippetQueryService.listRefundSnippets(tradeCompanyId, tradeId));
		out.remove("inital_request");
		out.remove("inital_response");
		putUsername(out, trade, tradeCompanyId);
		out.put("div_fee_info", adapayDivFeeInfoAssembler.build(trade, jwtMap));
		out.put("distributor_name", resolveDistributorName(tradeCompanyId, trade.getDistributorId()));
		out.put("mer_name", resolveMerNameForDivFeeRow(jwtCompanyId));
		return out;
	}

	/**
	 * Resolves the merchant display name for the company’s AdaPay entry row, or {@code "-"} when unset.
	 * Used for the trade payload and for admin-type split rows.
	 */
	public String resolveMerNameForDivFeeRow(long jwtCompanyId) {
		AdapayMerchantEntry row = adapayMerchantEntryMapper.selectOne(new LambdaQueryWrapper<AdapayMerchantEntry>()
				.eq(AdapayMerchantEntry::getCompanyId, jwtCompanyId)
				.last("LIMIT 1"));
		if (row == null || row.getMerName() == null || row.getMerName().isEmpty()) {
			return "-";
		}
		return row.getMerName();
	}
	...
	private void putUsername(Map<String, Object> out, Trade trade, long tradeCompanyId) {
		...
		Members m = membersMapper.selectOne(new LambdaQueryWrapper<Members>()
				...
				.eq(Members::getCompanyId, tradeCompanyId)
				.last("LIMIT 1"));
		if (m == null) {
			out.put("username", "");
			return;
		...
		MembersInfo mi = membersInfoMapper.selectOne(new LambdaQueryWrapper<MembersInfo>()
				...
				.eq(MembersInfo::getCompanyId, tradeCompanyId)
				.last("LIMIT 1"));
	...
	private String resolveDistributorName(long tradeCompanyId, String distributorIdRaw) {
		...
		Object info = distributorGetInfoSimpleService.getInfoSimple(tradeCompanyId, String.valueOf(did));
