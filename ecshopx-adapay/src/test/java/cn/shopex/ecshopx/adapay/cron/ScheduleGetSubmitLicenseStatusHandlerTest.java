...
class ScheduleGetSubmitLicenseStatusHandlerTest {

	private static final String SHORT = "adapay-get-submit-license-audit";
	...
	@InjectMocks
	private ScheduleGetSubmitLicenseStatusHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleGetSubmitLicenseStatusHandler.class);
	...
	@Test
	@DisplayName("handler 成功：无 CronAlertEvent，INFO 含 costMs、processed")
	void execute_ok_noAlert() {
		handler.execute();
		verify(openAccountService, times(1)).scheduleGetSubmitLicenseStatus();
	...
	void execute_failure_alertAndRethrow() {
		...
		doThrow(err).when(openAccountService).scheduleGetSubmitLicenseStatus();
