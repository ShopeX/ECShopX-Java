...
@Service
public class BspayTradeListForInfoQueryService {
	...
	private static final String SQL = """
			SELECT
			  ...
			  a.bspay_div_status AS bspayDivStatus,
			  a.bspay_fee_mode AS bspayFeeMode,
			  a.bspay_fee AS bspayFee,
			...
			GROUP BY
			  ...
			  a.distributor_id,
			  a.pay_channel,
			  c.order_auto_close_aftersales_time,
			  c.order_id
	...
	public BspayTradeListForInfoQueryService(NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
	...
	private static Map<String, Object> mapAggregatedRow(ResultSet rs) throws SQLException {
		...
		row.put("bspayDivStatus", rs.getString("bspayDivStatus"));
		row.put("bspayFeeMode", rs.getString("bspayFeeMode"));
		row.put("bspayFee", readIntOrZero(rs, "bspayFee"));
		...
		if (refundedFee <= 0L) {
			tradeState = "SUCCESS";
		} else if (refundedFee < (long) payFee) {
