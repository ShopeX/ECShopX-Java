...
@Service
public class WdtErpTradeAfterSaleNotificationProcessor {

	private static final Logger log = LoggerFactory.getLogger(WdtErpTradeAfterSaleNotificationProcessor.class);
	...
	private final WdtErpTradeAfterSaleAssemblePort wdtErpTradeAfterSaleAssemblePort;
	private final String aftersaleSyncMethod;

	public WdtErpTradeAfterSaleNotificationProcessor(
			...
			WdtErpTradeAfterSaleAssemblePort wdtErpTradeAfterSaleAssemblePort,
			@Value("${ecshopx.wdterp.methods.aftersale-sync:sales.Trade.aftersaleSync}") String aftersaleSyncMethod) {
		...
		this.wdtErpTradeAfterSaleAssemblePort = wdtErpTradeAfterSaleAssemblePort;
		this.aftersaleSyncMethod = aftersaleSyncMethod;
	}

	public void handle(Map<String, Object> erpAfterSalePayload) {
		if (erpAfterSalePayload == null || erpAfterSalePayload.isEmpty()) {
			return;
		}
		Long companyId = longVal(erpAfterSalePayload.get("company_id"));
		if (companyId == null || companyId <= 0) {
			return;
		}
		Map<String, Object> setting = wdtErpSettingReadPort.readWdtErpSetting(companyId);
		if (!isWdtOpen(setting)) {
			log.debug("wdterp trade aftersale skipped: ERP not enabled companyId={}", companyId);
			return;
		}
		long distributorId = longOrZero(erpAfterSalePayload.get("distributor_id"));
		String shopNo = resolveWdtShopNo(companyId, distributorId, setting);
		if (distributorId > 0 && !StringUtils.hasText(shopNo)) {
			log.debug(
					"wdterp trade aftersale skipped: distributor without wdt_shop_no companyId={} distributorId={}",
					companyId,
					distributorId);
			return;
		}
		if (!StringUtils.hasText(shopNo)) {
			log.debug("wdterp trade aftersale skipped: empty shop_no companyId={}", companyId);
			return;
		}
		List<List<Object>> bodies =
				wdtErpTradeAfterSaleAssemblePort.assembleAftersaleSyncBodies(companyId, erpAfterSalePayload, shopNo);
		if (bodies.isEmpty()) {
			log.debug("wdterp trade aftersale skipped: no sync payloads companyId={}", companyId);
			return;
		}
		...
		for (List<Object> bodyArgs : bodies) {
			try {
				Object result =
						wdtErpOpenApiClient.call(companyId, aftersaleSyncMethod, bodyArgs, sid, appKey, appSecret);
				if (isUnconfiguredClient(result)) {
					log.debug("wdterp trade aftersale skipped: client not configured companyId={}", companyId);
					return;
				}
				if (isHardFailure(result)) {
					log.warn(
							"wdterp trade aftersale non-success companyId={} detail={}",
							companyId,
							summarizeFailure(result));
				}
			} catch (RuntimeException e) {
				log.debug("wdterp trade aftersale request failed companyId={} msg={}", companyId, e.getMessage());
			}
		}
	}
