...
@Service
public class JushuitanTradeFinishNotificationProcessor {

	private static final Logger log = LoggerFactory.getLogger(JushuitanTradeFinishNotificationProcessor.class);
	...
	private final JushuitanTradeFinishOrderUploadAssemblePort jushuitanTradeFinishOrderUploadAssemblePort;

	public JushuitanTradeFinishNotificationProcessor(
			...
			JushuitanTradeFinishOrderUploadAssemblePort jushuitanTradeFinishOrderUploadAssemblePort) {
		...
		this.jushuitanTradeFinishOrderUploadAssemblePort = jushuitanTradeFinishOrderUploadAssemblePort;
	}
	...
	public void handle(Map<String, Object> tradeRowPayload) {
		...
		if (!isJushuitanOpen(setting)) {
			log.debug("jushuitan trade finish skipped: ERP not enabled companyId={}", companyId);
			return;
		}
		...
		if (distributorId > 0 && !StringUtils.hasText(jstShopId)) {
			log.debug(
					"jushuitan trade finish skipped: distributor without jst_shop_id companyId={} distributorId={}",
					companyId,
					distributorId);
			return;
		}
		List<Map<String, Object>> payloads =
				jushuitanTradeFinishOrderUploadAssemblePort.assembleOrderUploadPayloads(
						companyId, tradeRowPayload, jstShopId);
		if (payloads.isEmpty()) {
			log.debug("jushuitan trade finish skipped: no order upload payloads companyId={}", companyId);
			return;
		}
		String accessToken = setting.get("access_token") != null ? setting.get("access_token").toString() : "";
		for (Map<String, Object> biz : payloads) {
			Map<String, Object> result = jushuitanOpenApiClient.call(companyId, "order_upload", biz, accessToken);
			if (isUnconfiguredClient(result)) {
				log.debug("jushuitan trade finish skipped: client not configured companyId={}", companyId);
				return;
			}
			if (isHardFailure(result)) {
				throw new ResourceException("聚水潭订单同步失败: " + summarizeFailure(result));
			}
		}
	}
