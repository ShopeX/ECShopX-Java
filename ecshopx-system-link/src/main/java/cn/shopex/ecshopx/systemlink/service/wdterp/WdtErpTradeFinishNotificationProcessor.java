...
import cn.shopex.ecshopx.common.port.systemlink.WdtErpTradeFinishPushSelfAssemblePort;
...
@Service
public class WdtErpTradeFinishNotificationProcessor {

	private static final Logger log = LoggerFactory.getLogger(WdtErpTradeFinishNotificationProcessor.class);
	...
	private final WdtErpTradeFinishPushSelfAssemblePort wdtErpTradeFinishPushSelfAssemblePort;
	private final String pushSelfMethod;

	public WdtErpTradeFinishNotificationProcessor(
			...
			WdtErpTradeFinishPushSelfAssemblePort wdtErpTradeFinishPushSelfAssemblePort,
			@Value("${ecshopx.wdterp.methods.trade-finish-push-self:sales.RawTrade.pushSelf}") String pushSelfMethod) {
		...
		this.wdtErpTradeFinishPushSelfAssemblePort = wdtErpTradeFinishPushSelfAssemblePort;
		this.pushSelfMethod = pushSelfMethod;
	}
	...
	public void handle(Map<String, Object> tradeRowPayload) {
		...
		Long companyId = longVal(tradeRowPayload.get("company_id"));
		if (companyId == null || companyId <= 0) {
			return;
		}
		Map<String, Object> setting = wdtErpSettingReadPort.readWdtErpSetting(companyId);
		if (!isWdtOpen(setting)) {
			log.debug("wdterp trade finish skipped: ERP not enabled companyId={}", companyId);
			return;
		}
		long distributorId = longOrZero(tradeRowPayload.get("distributor_id"));
		String shopNo = resolveWdtShopNo(companyId, distributorId, setting);
		if (distributorId > 0 && !StringUtils.hasText(shopNo)) {
			log.debug(
					"wdterp trade finish skipped: distributor without wdt_shop_no companyId={} distributorId={}",
					companyId,
					distributorId);
			return;
		}
		if (!StringUtils.hasText(shopNo)) {
			log.debug("wdterp trade finish skipped: empty shop_no companyId={}", companyId);
			return;
		}
		List<List<Object>> bodies =
				wdtErpTradeFinishPushSelfAssemblePort.assemblePushSelfBodies(companyId, tradeRowPayload, shopNo);
		if (bodies.isEmpty()) {
			log.debug("wdterp trade finish skipped: no push payloads companyId={}", companyId);
			return;
		}
		...
		for (List<Object> bodyArgs : bodies) {
			try {
				Object result = wdtErpOpenApiClient.call(companyId, pushSelfMethod, bodyArgs, sid, appKey, appSecret);
				if (isUnconfiguredClient(result)) {
					log.debug("wdterp trade finish skipped: client not configured companyId={}", companyId);
					return;
				}
				if (isHardFailure(result)) {
					log.warn("wdterp trade finish non-success companyId={} detail={}", companyId, summarizeFailure(result));
				}
			} catch (RuntimeException e) {
				log.debug("wdterp trade finish request failed companyId={} msg={}", companyId, e.getMessage());
			}
		}
	}
