...
@DisplayName("EVENT_ORDER_PROCESS_LOG: admin POST /order/deliverystaff/confirm — publishEvent probe")
class AdminConfirmDeliveryStaffServiceOrderProcessLogDispatchPublishProbeTest {
	...
	private AdminConfirmDeliveryStaffService service;
	...
	void setUp() {
		...
		service =
				new AdminConfirmDeliveryStaffService(
		...
		row.setSelfDeliveryOperatorId(0L);
		...
		when(adminSelfDeliveryStaffFeeService.computeFeeFen(anyLong(), any(), anyLong())).thenReturn(500);
	}

	@Test
	void confirmDeliveryStaff_whenUpdateSucceeds_invokesPublishEventOnce_withDeliveryStaffAssignShapedPayload() {
		TransactionTemplate tt = new TransactionTemplate(syncFiringTxManager());

		tt.executeWithoutResult(
				status -> service.confirmDeliveryStaff(100L, "admin", 42L, "200", 99L));
		...
		assertThat(payload.get("remarks")).isEqualTo("商家已接单");
		...
		assertThat(inner.get("self_delivery_operator_id")).isEqualTo(99L);
	}
