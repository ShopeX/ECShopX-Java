...
@DisplayName("EVENT_ORDER_PROCESS_LOG: wxapp offline upload voucher async consume (slow)")
class WxappOfflinePaymentUploadVoucherOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowOfflineUploadShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		innerParams.put("voucher_pic", List.of("https://img.example/v.jpg"));
		innerParams.put("transfer_remark", "probe transfer note");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-wxapp-offline-upload-opl-async-consume",
						LISTENER_NAME);
