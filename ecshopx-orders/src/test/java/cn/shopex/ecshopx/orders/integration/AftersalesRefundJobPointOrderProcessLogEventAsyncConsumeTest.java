...
@DisplayName("EVENT_ORDER_PROCESS_LOG: RefundJob/point async consume (slow)")
class AftersalesRefundJobPointOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowPointRefundSuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-01-qc-refundjob-point-opl",
						LISTENER_NAME);
