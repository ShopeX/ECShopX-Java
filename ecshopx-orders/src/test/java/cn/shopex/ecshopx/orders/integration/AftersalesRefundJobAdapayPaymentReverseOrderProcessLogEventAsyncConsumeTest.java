...
@DisplayName("EVENT_ORDER_PROCESS_LOG: adapay payment reverse async consume (slow)")
class AftersalesRefundJobAdapayPaymentReverseOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowPaymentReverseSuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		expectedPayload.put("detail", "订单号：" + orderId + "，订单支付撤销成功（adapay支付渠道）");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-01-qc-refundjob-adapay-reverse-opl-ok",
						LISTENER_NAME);
	...
	@Test
	void consume_asyncSlowPaymentReverseFailShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		OrderProcessLogBusService bus = mock(OrderProcessLogBusService.class);
		OrderProcessLogDispatchListener listener = new OrderProcessLogDispatchListener(bus);
		...
		long companyId = 9203L;
		long orderId = 5203L;
		String err = "reverse declined";
		...
		expectedPayload.put("remarks", "订单退款");
		expectedPayload.put(
				"detail", "订单号：" + orderId + "，订单支付撤销（adapay支付渠道），失败原因：" + err);
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						RetryPolicy.platformDefault(),
						occurredAt,
						"trace-entry-01-qc-refundjob-adapay-reverse-opl-fail",
						LISTENER_NAME);
