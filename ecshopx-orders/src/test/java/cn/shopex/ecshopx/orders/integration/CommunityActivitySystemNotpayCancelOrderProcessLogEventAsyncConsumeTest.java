...
@DisplayName("EVENT_ORDER_PROCESS_LOG: community activity system NOTPAY cancel — DispatchConsumerRuntime consume (slow)")
class CommunityActivitySystemNotpayCancelOrderProcessLogEventAsyncConsumeTest {
	...
	@Test
	void consume_communityActivitySystemNotpayCancelShapedAsyncMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		LinkedHashMap<String, Object> innerParams = new LinkedHashMap<>();
		innerParams.put("other_reason", "活动结束自动取消");
		...
		expectedPayload.put("remarks", "取消订单");
		expectedPayload.put("detail", "订单号：200，来源：system");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-community-activity-system-notpay-cancel-opl-async-consume",
						LISTENER_NAME);
	...
	@Test
	void consume_communityActivityChiefNotpayCancelShapedAsyncMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		LinkedHashMap<String, Object> innerParams = new LinkedHashMap<>();
		innerParams.put("other_reason", "成团失败");
		...
		expectedPayload.put("remarks", "取消订单");
		expectedPayload.put("detail", "订单号：200，来源：chief");
		expectedPayload.put("is_show", Boolean.FALSE);
		expectedPayload.put("params", innerParams);

		Instant occurredAt = Instant.parse("2026-05-10T12:05:00Z");
		DispatchMessage msg =
				new DispatchMessage(
						DispatchMessageType.EVENT,
						DispatchMode.ASYNC,
						...
						RetryPolicy.platformDefault(),
						occurredAt,
						"trace-community-activity-chief-notpay-cancel-opl-async-consume",
						LISTENER_NAME);
