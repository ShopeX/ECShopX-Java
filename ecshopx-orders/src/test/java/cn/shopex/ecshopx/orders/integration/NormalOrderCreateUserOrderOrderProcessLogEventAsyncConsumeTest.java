...
@DisplayName("EVENT_ORDER_PROCESS_LOG: normal order createUserOrder (shopadmin) async consume (slow)")
class NormalOrderCreateUserOrderOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowShopadminCreateUserOrderShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		innerParams.put("order_type", "normal_shopadmin");
		innerParams.put("company_id", 12L);
		innerParams.put("operator_id", 99L);
		innerParams.put("order_source", "shop_offline");
		innerParams.put("source_from", "dianwu");
		...
		expectedPayload.put("operator_id", 5001L);
		expectedPayload.put("remarks", "订单创建");
		expectedPayload.put("detail", "订单号：70001，订单创建");
		expectedPayload.put("params", innerParams);

		Instant occurredAt = Instant.parse("2026-05-09T14:00:00Z");
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-shopadmin-create-user-order-opl-async-consume",
						LISTENER_NAME);
