...
class ScheduleCancelServiceGroupsOrdersHandlerTest {

	@Mock
	private GroupsServiceOrderService groupsServiceOrderService;

	@Mock
	private ApplicationEventPublisher eventPublisher;

	@InjectMocks
	private ScheduleCancelServiceGroupsOrdersHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleCancelServiceGroupsOrdersHandler.class);
	...
	void execute_ok_logsInfoAndNoAlert() {
		when(groupsServiceOrderService.scheduleCancelOrders()).thenReturn(3);
		handler.execute();
		verify(groupsServiceOrderService, times(1)).scheduleCancelOrders();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list).anyMatch(
				e -> e.getLevel() == Level.INFO
						&& e.getFormattedMessage() != null
						&& e.getFormattedMessage().contains("cancel-service-group-orders")
	...
	void execute_failure_publishesAlertAndRethrows() {
		...
		doThrow(err).when(groupsServiceOrderService).scheduleCancelOrders();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("cancel-service-group-orders");
		assertThat(listAppender.list).anyMatch(
