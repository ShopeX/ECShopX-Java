...
@DisplayName("EVENT_ORDER_PROCESS_LOG: RefundJob/chinaums async consume (slow)")
class AftersalesRefundJobChinaumsOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowChinaumsRefundSuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-01-qc-refundjob-chinaums-opl",
						LISTENER_NAME);
	...
	@Test
	void consume_asyncSlowChinaumsRefundFailShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		OrderProcessLogBusService bus = mock(OrderProcessLogBusService.class);
		OrderProcessLogDispatchListener listener = new OrderProcessLogDispatchListener(bus);
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						RetryPolicy.platformDefault(),
						occurredAt,
						"trace-entry-01-qc-refundjob-chinaums-opl-fail",
						LISTENER_NAME);
