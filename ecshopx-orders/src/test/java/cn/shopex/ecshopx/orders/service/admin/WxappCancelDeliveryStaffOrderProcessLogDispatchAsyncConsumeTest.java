...
@DisplayName("EVENT_ORDER_PROCESS_LOG: wxapp cancel delivery staff — DispatchConsumerRuntime consume (slow)")
class WxappCancelDeliveryStaffOrderProcessLogDispatchAsyncConsumeTest {
	...
	@Test
	void consume_wxappShapedAsyncMessage_invokesPersistEntitiesMapOnce() {
		long companyId = 100L;
		long orderId = 200L;
		long operatorId = 0L;
		...
		innerParams.put("operator_type", "");
		innerParams.put("self_delivery_status", "CONFIRMING");
		...
		expectedPayload.put("remarks", "配送取消");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-wxapp-cancel-delivery-staff-opl-async-consume",
						LISTENER_NAME);
