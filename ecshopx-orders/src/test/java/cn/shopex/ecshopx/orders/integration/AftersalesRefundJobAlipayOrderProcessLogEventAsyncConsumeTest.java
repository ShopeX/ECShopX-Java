...
@DisplayName("EVENT_ORDER_PROCESS_LOG: RefundJob/alipay async consume (slow)")
class AftersalesRefundJobAlipayOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowAlipayRefundSuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-01-qc-refundjob-alipay-opl",
						LISTENER_NAME);
	...
	@Test
	void consume_asyncSlowAlipayRefundFailShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		OrderProcessLogBusService bus = mock(OrderProcessLogBusService.class);
		OrderProcessLogDispatchListener listener = new OrderProcessLogDispatchListener(bus);
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						RetryPolicy.platformDefault(),
						occurredAt,
						"trace-entry-01-qc-refundjob-alipay-opl-fail",
						LISTENER_NAME);
