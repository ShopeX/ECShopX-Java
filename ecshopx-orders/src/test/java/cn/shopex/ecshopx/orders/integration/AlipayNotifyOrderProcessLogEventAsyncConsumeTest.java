...
@DisplayName("EVENT_ORDER_PROCESS_LOG: Alipay notify payment success async consume (slow)")
class AlipayNotifyOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowPaymentNotifyShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		expectedPayload.put("order_id", "501");
		expectedPayload.put("company_id", "9");
		expectedPayload.put("operator_type", "system");
		expectedPayload.put("is_show", Boolean.TRUE);
		expectedPayload.put("remarks", "订单支付");
		expectedPayload.put("detail", "订单号：501，订单支付成功");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-01-alipay-notify-opl-payment-success-async-consume",
						LISTENER_NAME);
