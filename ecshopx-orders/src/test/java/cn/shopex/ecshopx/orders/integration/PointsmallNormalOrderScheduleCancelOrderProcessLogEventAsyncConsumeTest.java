...
@DisplayName("EVENT_ORDER_PROCESS_LOG: pointsmall schedule cancel shaped async consume (slow)")
class PointsmallNormalOrderScheduleCancelOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowPointsmallScheduleCancelShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-pointsmall-schedule-cancel-opl-async-consume",
						LISTENER_NAME);
