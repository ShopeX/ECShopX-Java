...
import cn.shopex.ecshopx.orders.service.GroupsOrderService;
...
class ScheduleCancelNormalGroupOrdersHandlerTest {

	@Mock
	private GroupsOrderService groupsOrderService;

	@Mock
	private ApplicationEventPublisher eventPublisher;

	@InjectMocks
	private ScheduleCancelNormalGroupOrdersHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleCancelNormalGroupOrdersHandler.class);
	...
	void execute_ok_logsInfoAndNoAlert() {
		when(groupsOrderService.scheduleCancelOrders()).thenReturn(3);
		handler.execute();
		verify(groupsOrderService, times(1)).scheduleCancelOrders();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list).anyMatch(
				e -> e.getLevel() == Level.INFO
						&& e.getFormattedMessage() != null
						&& e.getFormattedMessage().contains("cancel-group-orders")
	...
	void execute_failure_publishesAlertAndRethrows() {
		...
		doThrow(err).when(groupsOrderService).scheduleCancelOrders();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("cancel-group-orders");
		assertThat(listAppender.list).anyMatch(
