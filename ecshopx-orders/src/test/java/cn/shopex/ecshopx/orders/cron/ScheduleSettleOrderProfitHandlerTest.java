...
class ScheduleSettleOrderProfitHandlerTest {
	...
	@InjectMocks
	private ScheduleSettleOrderProfitHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleSettleOrderProfitHandler.class);
	...
	@Test
	@DisplayName("handler 正常 path plan §5；经 service 完成 analysis §3 步骤 1–10，INFO 含 processed")
	void execute_ok_logsInfoAndNoAlert() {
		when(orderProfitService.scheduleSettleProfit()).thenReturn(3);
		handler.execute();
		verify(orderProfitService, times(1)).scheduleSettleProfit();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list).anyMatch(
				e -> e.getLevel() == Level.INFO
						&& e.getFormattedMessage() != null
						&& e.getFormattedMessage().contains("settle-order-profit")
						&& e.getFormattedMessage().contains("cost=")
						&& e.getFormattedMessage().contains("processed=3"));
	}

	@Test
	@DisplayName("handler 异常 path plan §5；ERROR 含 costMs 与堆栈、CronAlertEvent（analysis §3 在 service 内中断）")
	void execute_failure_publishesAlertAndRethrows() {
		RuntimeException err = new RuntimeException("test");
		doThrow(err).when(orderProfitService).scheduleSettleProfit();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("settle-order-profit");
