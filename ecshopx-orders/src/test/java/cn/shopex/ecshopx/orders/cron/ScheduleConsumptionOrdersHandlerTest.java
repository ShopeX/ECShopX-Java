...
class ScheduleConsumptionOrdersHandlerTest {
	...
	@InjectMocks
	private ScheduleConsumptionOrdersHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleConsumptionOrdersHandler.class);
	...
	/** analysis §3 1.2 — 仅 wiring */
	@Test
	void execute_ok_callsScheduleConsumptionOrders() {
		when(normalOrderCronService.scheduleConsumptionOrders()).thenReturn(1);
		handler.execute();
		verify(normalOrderCronService, times(1)).scheduleConsumptionOrders();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list).anyMatch(
				e -> e.getLevel() == Level.INFO
						&& e.getFormattedMessage() != null
						&& e.getFormattedMessage().contains("consumption-orders")
						&& e.getFormattedMessage().contains("scheduleConsumptionOrdersReturn=1"));
	}
	...
	void execute_failure_publishesAlertAndRethrows() {
		...
		doThrow(err).when(normalOrderCronService).scheduleConsumptionOrders();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("consumption-orders");
	}
}
