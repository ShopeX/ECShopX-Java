...
@DisplayName("EVENT_ORDER_PROCESS_LOG: admin POST /order/cancel/deliverystaff — publishEvent probe")
class AdminCancelDeliveryStaffServiceOrderProcessLogDispatchPublishProbeTest {
	...
	@Test
	void cancelDeliveryStaff_whenUpdateSucceeds_invokesPublishEventOnce_withCancelDeliveryOplShapedPayload() {
		TransactionTemplate tt = new TransactionTemplate(syncFiringTxManager());

		tt.executeWithoutResult(status -> service.cancelDeliveryStaff(100L, "admin", 42L, "200"));
