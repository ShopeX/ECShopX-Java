...
@DisplayName("EVENT_ORDER_PROCESS_LOG: front POST wxapp order cancel deliverystaff — publishEvent probe")
class WxappCancelDeliveryStaffOrderProcessLogDispatchPublishProbeTest {
	...
	private AdminCancelDeliveryStaffService service;
	...
	void setUp() {
		...
		service =
				new AdminCancelDeliveryStaffService(
		...
		row.setOrderId(200L);
		row.setSelfDeliveryStatus("RECEIVEORDER");
		when(normalOrdersMapper.selectOne(any())).thenReturn(row);
		when(normalOrdersMapper.update(any(), any())).thenReturn(1);
	}

	@Test
	void cancelDeliveryStaff_whenWxappShapedArgsAndUpdateSucceeds_invokesPublishEventOnce_withH5OplShapedPayload() {
		TransactionTemplate tt = new TransactionTemplate(syncFiringTxManager());

		tt.executeWithoutResult(status -> service.cancelDeliveryStaff(100L, "", 0L, "200"));
		...
		assertThat(payload.get("remarks")).isEqualTo("配送取消");
		assertThat(payload.get("detail")).isEqualTo("订单号：200，接单分配配送员");
		...
		assertThat(inner.get("operator_id")).isEqualTo(0L);
		assertThat(inner.get("operator_type")).isEqualTo("");
		assertThat(inner.get("self_delivery_status")).isEqualTo("CONFIRMING");
	}
