...
/**
 * Async consume path for {@link OrdersDispatchEventNames#EVENT_ORDER_PROCESS_LOG} shaped like the kernel
 * {@code schedule_cancel_normals_orders} cancel flow payload (remarks/detail/operator fields).
 */
@DisplayName("EVENT_ORDER_PROCESS_LOG: schedule cancel normals shaped async consume (slow)")
class ScheduleCancelNormalOrdersOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowScheduleCancelNormalsShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-schedule-cancel-normal-orders-opl-async-consume",
						LISTENER_NAME);
