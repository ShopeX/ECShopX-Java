...
@DisplayName("EVENT_ORDER_PROCESS_LOG: resubmitRefund/wxpay async consume (slow)")
class RefundErrorLogsResubmitRefundWechatOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowResubmitRefundWechatSuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		expectedPayload.put("detail", "订单号：" + orderId + "，订单退款成功（微信支付渠道）");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-02-resubmit-wechat-opl-success",
						LISTENER_NAME);
	...
	@Test
	void consume_asyncSlowResubmitRefundWechatFailShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		OrderProcessLogBusService bus = mock(OrderProcessLogBusService.class);
		OrderProcessLogDispatchListener listener = new OrderProcessLogDispatchListener(bus);
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						RetryPolicy.platformDefault(),
						occurredAt,
						"trace-entry-02-resubmit-wechat-opl-fail",
						LISTENER_NAME);
