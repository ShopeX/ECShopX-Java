...
@DisplayName(
		"EVENT_ORDER_PROCESS_LOG: wxapp POST /order/deliverypackag/confirm — publishEvent probe (system/0 operator)")
class WxappConfirmDeliveryPackagOrderProcessLogDispatchPublishProbeTest {
	...
	@Test
	void confirmDeliveryPackag_whenWxappOperatorDefaults_invokesPublishEventOnce_withPackagedOplPayload() {
		TransactionTemplate tt = new TransactionTemplate(syncFiringTxManager());

		tt.executeWithoutResult(status -> service.confirmDeliveryPackag(100L, "", 0L, "200"));
		...
		assertThat(inner.get("operator_type")).isEqualTo("system");
