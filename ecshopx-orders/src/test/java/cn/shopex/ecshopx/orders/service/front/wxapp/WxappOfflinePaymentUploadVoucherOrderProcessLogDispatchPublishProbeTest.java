...
@DisplayName("EVENT_ORDER_PROCESS_LOG: wxapp offline upload voucher publishEvent probe")
class WxappOfflinePaymentUploadVoucherOrderProcessLogDispatchPublishProbeTest {
	...
	private WxappOfflinePaymentUploadVoucherService service;
	...
	void setUp() {
		...
		service = new WxappOfflinePaymentUploadVoucherService(
	...
	@Test
	void uploadVoucher_success_invokesPublishEventOnce_withOfflineTransferPayload() {
		long companyId = 11L;
		long userId = 22L;
		...
		merged.put("voucher_pic", List.of("https://img.example/v.jpg"));
		merged.put("transfer_remark", "probe transfer note");
		...
		when(offlinePaymentMapper.selectOne(any())).thenReturn(null);
		...
		when(offlinePaymentMapper.insert(any(OfflinePayment.class))).thenReturn(1);
		when(normalOrdersMapper.update(isNull(), any())).thenReturn(1);

		service.uploadOfflineVoucher(httpServletRequest, merged, auth);
