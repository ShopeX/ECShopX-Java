 ...
 * for the pointsmall confirm-cancel reject path and asserts {@link OrderProcessLogDispatchListener} delegates to
 * {@link OrderProcessLogBusService}.
 */
@DisplayName("EVENT_ORDER_PROCESS_LOG: pointsmall confirm cancel reject async consume (slow)")
class PointsmallConfirmCancelRejectOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowPointsmallConfirmCancelRejectOplShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		requestParams.put("check_cancel", "0");
		requestParams.put("order_type", "normal_pointsmall");
		requestParams.put("refund_bn", "800");
		requestParams.put("shop_reject_reason", shopRejectReason);
		...
		LinkedHashMap<String, Object> expectedPayload =
				PointsmallApiConfirmCancelRejectOrderProcessLogEntities.buildForAdminRejectRefund(
						orderId, companyId, 0L, "", shopRejectReason, requestParams);
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-pointsmall-confirm-cancel-reject-opl-async-consume",
						LISTENER_NAME);
