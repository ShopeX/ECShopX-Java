 ...
 * the offline-pay timeout cancel batch order-process-log branch (remarks/detail/operator fields).
 */
@DisplayName("EVENT_ORDER_PROCESS_LOG: offline pay cancel normals shaped async consume (slow)")
class ScheduleOfflinePayCancelNormalsLogOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowOfflinePayCancelNormalsOplShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-offline-pay-cancel-normals-opl-async-consume",
						LISTENER_NAME);
