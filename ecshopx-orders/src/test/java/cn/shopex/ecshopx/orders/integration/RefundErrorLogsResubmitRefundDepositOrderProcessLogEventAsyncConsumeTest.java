...
@DisplayName("EVENT_ORDER_PROCESS_LOG: resubmitRefund/deposit async consume (slow)")
class RefundErrorLogsResubmitRefundDepositOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowResubmitRefundDepositSuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		expectedPayload.put("detail", "订单号：" + orderId + "，订单退款成功（储值金额渠道)");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-02-resubmit-refund-deposit-opl",
						LISTENER_NAME);
