...
@DisplayName("EVENT_ORDER_PROCESS_LOG: admin offline do_check NOTPAY→SUCCESS async consume (slow)")
class OfflinePaymentDoCheckTradeSuccessOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowAdminOfflineTradeSuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		expectedPayload.put("remarks", "交易支付成功");
		expectedPayload.put("detail", "NOTPAY→SUCCESS");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-offline-docheck-trade-success-opl-async-consume",
						LISTENER_NAME);
