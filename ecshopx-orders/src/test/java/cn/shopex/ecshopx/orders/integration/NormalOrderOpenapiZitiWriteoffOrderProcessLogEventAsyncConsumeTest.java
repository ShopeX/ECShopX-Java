...
@DisplayName("EVENT_ORDER_PROCESS_LOG: OpenAPI ziti writeoff async consume (slow)")
class NormalOrderOpenapiZitiWriteoffOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowOpenapiZitiWriteoffShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		innerParams.put("operator_type", "openapi");
		innerParams.put("operator_id", 0L);
		...
		expectedPayload.put("operator_type", "openapi");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-openapi-ziti-writeoff-opl-async-consume",
						LISTENER_NAME);
