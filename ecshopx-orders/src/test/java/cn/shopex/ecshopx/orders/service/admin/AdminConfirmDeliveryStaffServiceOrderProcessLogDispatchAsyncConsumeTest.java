...
@DisplayName("EVENT_ORDER_PROCESS_LOG: admin confirm delivery staff assign — DispatchConsumerRuntime consume (slow)")
class AdminConfirmDeliveryStaffServiceOrderProcessLogDispatchAsyncConsumeTest {
	...
	@Test
	void consume_deliveryStaffConfirmShapedAsyncMessage_invokesPersistEntitiesMapOnce() {
		long companyId = 100L;
		long orderId = 200L;
		long operatorId = 42L;
		long selfDeliveryOperatorId = 99L;
		...
		innerParams.put("self_delivery_operator_id", selfDeliveryOperatorId);
		...
		expectedPayload.put("remarks", "商家已接单");
		expectedPayload.put("detail", "订单号：200，接单分配配送员");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-admin-confirm-delivery-staff-opl-async-consume",
						LISTENER_NAME);
