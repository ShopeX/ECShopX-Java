...
@DisplayName(
		"Admin confirm-cancel reject READY refund dispatches order process log; async consume matches expected payload")
class AdminOrderPassRefundServiceAdminApiRejectRefundReadyOrderProcessLogEventAsyncConsumeTest {
	...
	@Test
	void consume_adminApiRejectRefundReadyAsyncMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		LinkedHashMap<String, Object> innerParams = new LinkedHashMap<>();
		innerParams.put("company_id", 1L);
		innerParams.put("order_id", 200L);
		innerParams.put("check_cancel", "0");
		innerParams.put("order_type", "normal");
		innerParams.put("operator_type", "shop");
		innerParams.put("operator_id", 7L);
		innerParams.put("shop_reject_reason", "档期已满");
		...
		expectedPayload.put("operator_type", "shop");
		expectedPayload.put("operator_id", 7L);
		expectedPayload.put("remarks", "订单退款");
		expectedPayload.put("detail", "订单号：200，后台管理员拒绝退款，拒绝原因：档期已满");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-admin-api-reject-refund-ready-opl-async-consume",
						LISTENER_NAME);
