...
class ScheduleUpdateRightStatusHandlerTest {
	...
	@InjectMocks
	private ScheduleUpdateRightStatusHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleUpdateRightStatusHandler.class);
	...
	void execute_ok_noAlert() {
		when(timesCardService.scheduleUpdateRightStatus()).thenReturn(5);
		handler.execute();
		verify(timesCardService, times(1)).scheduleUpdateRightStatus();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list).anyMatch(
				e -> e.getLevel() == Level.INFO
						&& e.getFormattedMessage() != null
						&& e.getFormattedMessage().contains("update-rights-status")
						&& e.getFormattedMessage().contains("cost=")
						&& e.getFormattedMessage().contains("rowsAffectedApprox=5"));
	}

	@Test
	void execute_failure_publishesAlertAndRethrows() {
		RuntimeException err = new RuntimeException("test");
		doThrow(err).when(timesCardService).scheduleUpdateRightStatus();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("update-rights-status");
