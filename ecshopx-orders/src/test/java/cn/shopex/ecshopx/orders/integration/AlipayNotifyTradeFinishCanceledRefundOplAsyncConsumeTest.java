...
@DisplayName("Order process log: Alipay notify trade finish canceled refund — async slow consume")
class AlipayNotifyTradeFinishCanceledRefundOplAsyncConsumeTest {
	...
	void consume_slowQueueRefundShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		Instant occurredAt = Instant.parse("2026-05-10T08:15:30Z");
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-alipay-notify-trade-finish-canceled-refund-opl-async-consume",
						LISTENER_NAME);
