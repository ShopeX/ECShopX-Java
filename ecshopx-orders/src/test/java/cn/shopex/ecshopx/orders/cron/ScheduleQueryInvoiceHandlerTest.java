...
class ScheduleQueryInvoiceHandlerTest {
	...
	@InjectMocks
	private ScheduleQueryInvoiceHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleQueryInvoiceHandler.class);
	...
	void execute_ok_noAlert() {
		when(orderInvoiceService.scheduleQueryInvoice()).thenReturn(2);
		handler.execute();
		verify(orderInvoiceService, times(1)).scheduleQueryInvoice();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								&& e.getFormattedMessage() != null
								&& e.getFormattedMessage().contains("query-invoice")
								&& e.getFormattedMessage().contains("dispatched=2"));
	}

	@Test
	void execute_failure_publishesAlertAndRethrows() {
		RuntimeException err = new RuntimeException("test");
		doThrow(err).when(orderInvoiceService).scheduleQueryInvoice();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("query-invoice");
		assertThat(listAppender.list)
				.anyMatch(
