...
@DisplayName("EVENT_ORDER_PROCESS_LOG: resubmitRefund/point async consume (slow)")
class RefundErrorLogsResubmitRefundPointOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowResubmitRefundPointSuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		expectedPayload.put("detail", "订单号：" + orderId + "，订单退还积分成功");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-02-resubmit-refund-point-opl",
						LISTENER_NAME);
