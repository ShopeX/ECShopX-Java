...
@DisplayName("EVENT_ORDER_PROCESS_LOG: RefundJob/wxpay async consume (slow)")
class AftersalesRefundJobWechatOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowWechatRefundSuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-01-qc-refundjob-wechat-opl",
						LISTENER_NAME);
	...
	@Test
	void consume_asyncSlowWechatRefundFailShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		OrderProcessLogBusService bus = mock(OrderProcessLogBusService.class);
		OrderProcessLogDispatchListener listener = new OrderProcessLogDispatchListener(bus);
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						RetryPolicy.platformDefault(),
						occurredAt,
						"trace-entry-01-qc-refundjob-wechat-opl-fail",
						LISTENER_NAME);
