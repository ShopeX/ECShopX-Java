...
@DisplayName("EVENT_ORDER_PROCESS_LOG: resubmitRefund/bspay async consume (slow)")
class RefundErrorLogsResubmitRefundBspayOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowResubmitRefundBspaySuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-02-resubmit-bspay-opl-success",
						LISTENER_NAME);
	...
	@Test
	void consume_asyncSlowResubmitRefundBspayFailShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		OrderProcessLogBusService bus = mock(OrderProcessLogBusService.class);
		OrderProcessLogDispatchListener listener = new OrderProcessLogDispatchListener(bus);
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						RetryPolicy.platformDefault(),
						occurredAt,
						"trace-entry-02-resubmit-bspay-opl-fail",
						LISTENER_NAME);
