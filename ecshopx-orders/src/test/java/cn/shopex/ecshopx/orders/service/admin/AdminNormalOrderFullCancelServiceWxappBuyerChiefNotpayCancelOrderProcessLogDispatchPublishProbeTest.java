...
@DisplayName("EVENT_ORDER_PROCESS_LOG: wxapp cancel NOTPAY+PENDING — publishEvent probe (buyer|chief)")
class AdminNormalOrderFullCancelServiceWxappBuyerChiefNotpayCancelOrderProcessLogDispatchPublishProbeTest {
	...
	void notPay_buyerCancel_afterCommit_invokesPublishEventWithWxappShapedOplPayload() {
		...
		runNotPayCancelInTx(svc, merged, "buyer");
		...
		assertThat(payload.get("detail")).isEqualTo("订单号：200，来源：buyer");
	...
	@Test
	@SuppressWarnings("unchecked")
	void notPay_chiefCancel_afterCommit_invokesPublishEventWithWxappShapedOplPayload() {
		DispatchFacade dispatchFacade = mock(DispatchFacade.class);
		AdminNormalOrderFullCancelService svc = buildServiceForNotPayCancel(dispatchFacade);
		...
		runNotPayCancelInTx(svc, merged, "chief");
	...
	private static void runNotPayCancelInTx(
