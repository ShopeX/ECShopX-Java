...
@DisplayName("EVENT_ORDER_PROCESS_LOG: admin updateDelivery merchant branch — DispatchConsumerRuntime consume")
class AdminOrderUpdateDeliveryMerchantBranchOrderProcessLogDispatchAsyncConsumeTest {
	...
	@Test
	void consume_merchantUpdateDeliveryShapedAsyncMessage_invokesPersistEntitiesMapOnce() {
		long companyId = 100L;
		long orderId = 200L;
		...
		innerParams.put("delivery_remark", "轻放");
		innerParams.put("delivery_pics", List.of("http://example.com/p1.png"));
		...
		expectedPayload.put("remarks", "已打包");
		expectedPayload.put("detail", "订单号：" + orderId + "，订单发货信息修改");
		expectedPayload.put("delivery_remark", "轻放");
		expectedPayload.put("pics", List.of("http://example.com/p1.png"));
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-admin-update-delivery-merchant-opl-async-consume",
						LISTENER_NAME);
