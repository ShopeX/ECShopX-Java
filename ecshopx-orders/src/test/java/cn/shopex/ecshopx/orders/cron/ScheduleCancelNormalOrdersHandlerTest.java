...
class ScheduleCancelNormalOrdersHandlerTest {
	...
	@InjectMocks
	private ScheduleCancelNormalOrdersHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleCancelNormalOrdersHandler.class);
	...
	void execute_ok_logsInfoAndNoAlert() {
		when(normalOrderCronService.scheduleCancelOrders()).thenReturn(3);
		handler.execute();
		verify(normalOrderCronService, times(1)).scheduleCancelOrders();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list).anyMatch(
				e -> e.getLevel() == Level.INFO
						&& e.getFormattedMessage() != null
						&& e.getFormattedMessage().contains("cancel-orders")
	...
	void execute_failure_publishesAlertAndRethrows() {
		...
		doThrow(err).when(normalOrderCronService).scheduleCancelOrders();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("cancel-orders");
		assertThat(listAppender.list).anyMatch(
