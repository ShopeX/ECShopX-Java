...
/**
 * Publish-side probe for {@link NormalOrderCronService#scheduleCancelOrders()} order-process-log path
 * (kernel schedule cancel normals), aligned with queued listener payload expectations.
 */
@ExtendWith(MockitoExtension.class)
class NormalOrderCronServiceScheduleCancelNormalOrdersOrderProcessLogDispatchPublishProbeTest {
	...
	@Test
	void scheduleCancelOrders_invokesOrderProcessLogPublishPortOnce_withKernelScheduleCancelNormalsPayload() {
		long orderId = 100L;
		long companyId = 1L;
		...
		verify(orderInvoiceCancelOnOrderCancelService, times(1))
				.updateInvoiceStatusCancel(eq(companyId), eq(orderId), eq("cancel"));
	}
}
