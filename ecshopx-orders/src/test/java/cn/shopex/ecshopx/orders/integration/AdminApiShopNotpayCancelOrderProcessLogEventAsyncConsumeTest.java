...
@DisplayName("EVENT_ORDER_PROCESS_LOG: admin shop NOTPAY cancel — DispatchConsumerRuntime consume (slow)")
class AdminApiShopNotpayCancelOrderProcessLogEventAsyncConsumeTest {
	...
	@Test
	void consume_adminShopNotpayCancelShapedAsyncMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		expectedPayload.put("detail", "订单号：200，来源：shop");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-admin-api-shop-notpay-cancel-opl-async-consume",
						LISTENER_NAME);
