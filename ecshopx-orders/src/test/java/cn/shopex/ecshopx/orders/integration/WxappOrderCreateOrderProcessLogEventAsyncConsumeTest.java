...
@DisplayName("EVENT_ORDER_PROCESS_LOG: wxapp order create async consume (slow)")
class WxappOrderCreateOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowWxappOrderCreatedShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		innerParams.put("company_id", 400L);

		LinkedHashMap<String, Object> expectedPayload = new LinkedHashMap<>();
		expectedPayload.put("order_id", "9001");
		expectedPayload.put("company_id", "400");
		expectedPayload.put("operator_type", "user");
		expectedPayload.put("is_show", Boolean.TRUE);
		expectedPayload.put("operator_id", 500L);
		expectedPayload.put("remarks", "订单创建");
		expectedPayload.put("detail", "订单号：9001，订单创建");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-wxapp-order-create-opl-async-consume",
						LISTENER_NAME);
