...
@DisplayName("EVENT_ORDER_PROCESS_LOG: resubmitRefund/adapay async consume (slow)")
class RefundErrorLogsResubmitRefundAdapayOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowResubmitRefundSuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-02-resubmit-adapay-success",
						LISTENER_NAME);
	...
	@Test
	void consume_asyncSlowResubmitRefundFailShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		OrderProcessLogBusService bus = mock(OrderProcessLogBusService.class);
		OrderProcessLogDispatchListener listener = new OrderProcessLogDispatchListener(bus);
		...
		long orderId = 5203L;
		String err = "AdaPay refund declined (resubmit consume)";
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						RetryPolicy.platformDefault(),
						occurredAt,
						"trace-entry-02-resubmit-adapay-fail",
						LISTENER_NAME);
