...
@DisplayName("EVENT_ORDER_PROCESS_LOG: resubmitRefund/chinaums async consume (slow)")
class RefundErrorLogsResubmitRefundChinaumsOrderProcessLogEventAsyncConsumeTest {

	private static final String LISTENER_NAME = "listener:orders.listeners.OrderProcess.OrderProcessLog";

	@Test
	void consume_asyncSlowResubmitRefundChinaumsSuccessShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		...
		expectedPayload.put("detail", "订单号：" + orderId + "，订单退款成功（银联支付渠道）");
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						occurredAt,
						"trace-entry-02-resubmit-chinaums-opl-success",
						LISTENER_NAME);
	...
	@Test
	void consume_asyncSlowResubmitRefundChinaumsFailShapedMessage_invokesOrderProcessLogBusServicePersistEntitiesMapOnce() {
		OrderProcessLogBusService bus = mock(OrderProcessLogBusService.class);
		OrderProcessLogDispatchListener listener = new OrderProcessLogDispatchListener(bus);
		...
		long companyId = 9207L;
		long orderId = 5207L;
		String err = "网关超时（resubmit）";
		...
		expectedPayload.put("remarks", "订单退款");
		expectedPayload.put(
				"detail", "订单号：" + orderId + "，订单退款失败（银联支付渠道），失败原因：" + err);
		...
		DispatchMessage msg =
				new DispatchMessage(
						...
						RetryPolicy.platformDefault(),
						occurredAt,
						"trace-entry-02-resubmit-chinaums-opl-fail",
						LISTENER_NAME);
