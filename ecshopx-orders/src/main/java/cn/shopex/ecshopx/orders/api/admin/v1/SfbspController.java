...
@AdminAuth
@ShopLog
@RestController("ordersAdminV1Sfbsp")
@RequestMapping("/api/v1/trade/sfbsp")
public class SfbspController {

	private final SfbspSettingRedisService sfbspSettingRedisService;

	public SfbspController(SfbspSettingRedisService sfbspSettingRedisService) {
		this.sfbspSettingRedisService = sfbspSettingRedisService;
	}

	@Activated(routeAlias = "trade.sfbsp.setting.set")
	@PostMapping(value = "/setting", name = "顺丰BSP设置保存", produces = MediaType.APPLICATION_JSON_VALUE)
	public ApiResult<Map<String, Object>> setSfbspSetting(
		...
		sfbspSettingRedisService.setSfbspSetting(companyId, configRaw);
		LinkedHashMap<String, Object> data = new LinkedHashMap<>(1);
		data.put("status", Boolean.TRUE);
		return ApiResult.ok(data);
	}

	@Activated(routeAlias = "trade.sfbsp.setting.get")
	@GetMapping(value = "/setting", name = "顺丰BSP设置")
	public ApiResult<JsonNode> getSfbspSetting(HttpServletRequest request) {
		long companyId = readCompanyIdFromJwt(request);
		JsonNode data = sfbspSettingRedisService.getSfbspSetting(companyId);
		return ApiResult.ok(data);
	}
