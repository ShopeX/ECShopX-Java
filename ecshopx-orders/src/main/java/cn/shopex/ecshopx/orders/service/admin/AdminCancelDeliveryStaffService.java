...
@Service
public class AdminCancelDeliveryStaffService {
	...
	public AdminCancelDeliveryStaffService(
	...
	public void cancelDeliveryStaff(
		...
		if ("CONFIRMING".equals(sds) || "DONE".equals(sds)) {
			throw new ResourceException("当前状态不可以取消配送");
		}

		String writeStatus = sds;
		if ("RECEIVEORDER".equals(sds)) {
		...
		params.put("self_delivery_status", writeStatus);

		int now = (int) (System.currentTimeMillis() / 1000L);
		LambdaUpdateWrapper<NormalOrders> u = new LambdaUpdateWrapper<>();
		u.eq(NormalOrders::getCompanyId, companyId)
				.eq(NormalOrders::getOrderId, orderIdNum)
				.set(NormalOrders::getSelfDeliveryOperatorId, 0L)
				.set(NormalOrders::getSelfDeliveryFee, 0)
				.set(NormalOrders::getSelfDeliveryStatus, writeStatus)
		...
		String ot = (String) params.get("operator_type");
		log.put(
				"operator_type",
				(ot == null || ot.isEmpty()) ? "system" : ot);
		log.put("operator_id", operatorId);
		log.put("is_show", Boolean.TRUE);
		log.put("remarks", "配送取消");
