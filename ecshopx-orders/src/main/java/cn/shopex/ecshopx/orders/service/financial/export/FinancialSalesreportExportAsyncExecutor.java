...
@Service
public class FinancialSalesreportExportAsyncExecutor {

	private static final Logger log = LoggerFactory.getLogger(FinancialSalesreportExportAsyncExecutor.class);

	private final FinancialSalesreportExportFileJobHandler financialSalesreportExportFileJobHandler;

	public FinancialSalesreportExportAsyncExecutor(
	...
	@Async("ordersFinancialSalesreportExportExecutor")
	public void executeAsync(FinancialSalesreportExportJobContext ctx) {
		try {
			financialSalesreportExportFileJobHandler.run(ctx);
		} catch (Exception e) {
			log.error("financial salesreport export async failed", e);
		}
	}
}
