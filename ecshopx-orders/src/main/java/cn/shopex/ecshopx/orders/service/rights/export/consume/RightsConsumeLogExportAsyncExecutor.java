...
@Service
public class RightsConsumeLogExportAsyncExecutor {

	private static final Logger log = LoggerFactory.getLogger(RightsConsumeLogExportAsyncExecutor.class);

	private final RightsConsumeLogExportFileJobHandler rightsConsumeLogExportFileJobHandler;

	public RightsConsumeLogExportAsyncExecutor(
	...
	@Async("ordersRightsExportExecutor")
	public void executeAsync(RightsConsumeLogJobContext ctx) {
		try {
			rightsConsumeLogExportFileJobHandler.run(ctx);
		} catch (Exception e) {
			log.error("rights consume log export async failed", e);
		}
	}
}
