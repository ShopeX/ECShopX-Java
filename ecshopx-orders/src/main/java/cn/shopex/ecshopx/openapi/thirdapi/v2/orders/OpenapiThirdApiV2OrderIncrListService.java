...
@Service
public class OpenapiThirdApiV2OrderIncrListService {
	...
	public OpenapiThirdApiV2OrderIncrListService(
	...
	private LambdaQueryWrapper<NormalOrders> buildWrapper(OrderFilter filter) {
		...
		if (filter.updateTimeGte != null) {
			wrapper.ge(NormalOrders::getUpdateTime, filter.updateTimeGte);
		}
		if (filter.updateTimeLte != null) {
			wrapper.le(NormalOrders::getUpdateTime, filter.updateTimeLte);
		}
		...
		return wrapper.orderByDesc(NormalOrders::getUpdateTime);
	}
