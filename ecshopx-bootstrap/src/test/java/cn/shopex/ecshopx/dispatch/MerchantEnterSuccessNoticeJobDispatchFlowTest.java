...
class MerchantEnterSuccessNoticeJobDispatchFlowTest {
	...
	void dispatchJob_async_enqueuesOnSmsQueue_andConsumerInvokesJobHandler() {
		...
		MerchantEnterSuccessNoticeJobHandler handler = new MerchantEnterSuccessNoticeJobHandler(sms);

		InMemoryDispatchRegistry registry = new InMemoryDispatchRegistry();
		registry.registerJob(MerchantDispatchJobNames.MERCHANT_ENTER_SUCCESS_NOTICE, handler);
		...
		facade.dispatchJob(
				MerchantDispatchJobNames.MERCHANT_ENTER_SUCCESS_NOTICE,
		...
		assertEquals(MerchantDispatchJobNames.MERCHANT_ENTER_SUCCESS_NOTICE, msg.messageName());
		...
		verify(sms)
				.sendTemplatedNoticeSms(
						eq(1L),
						eq("13800138000"),
						eq("merchant_enter_success_notice"),
						argThat(
								m ->
										m != null
												&& "123456".equals(m.get("password"))
