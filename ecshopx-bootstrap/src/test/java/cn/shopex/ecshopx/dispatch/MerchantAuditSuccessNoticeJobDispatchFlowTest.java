...
class MerchantAuditSuccessNoticeJobDispatchFlowTest {
	...
	void dispatchJob_async_enqueuesOnSmsQueue_andConsumerInvokesJobHandler() {
		...
		MerchantAuditSuccessNoticeJobHandler handler = new MerchantAuditSuccessNoticeJobHandler(sms);

		InMemoryDispatchRegistry registry = new InMemoryDispatchRegistry();
		registry.registerJob(MerchantDispatchJobNames.MERCHANT_AUDIT_SUCCESS_NOTICE, handler);
		...
		facade.dispatchJob(
				MerchantDispatchJobNames.MERCHANT_AUDIT_SUCCESS_NOTICE,
		...
		assertEquals(MerchantDispatchJobNames.MERCHANT_AUDIT_SUCCESS_NOTICE, msg.messageName());
		...
		verify(sms)
				.sendTemplatedNoticeSms(
						eq(1L),
						eq("13800138000"),
						eq("merchant_audit_success_notice"),
