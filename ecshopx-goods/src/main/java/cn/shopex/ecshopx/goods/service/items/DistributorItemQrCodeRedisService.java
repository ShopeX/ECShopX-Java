...
@Service
public class DistributorItemQrCodeRedisService {

	private final StringRedisTemplate sharedStringRedisTemplate;

	public DistributorItemQrCodeRedisService(
	...
	public Optional<String> hashGet(long companyId, long distributorId, long itemId) {
		Object raw =
				sharedStringRedisTemplate
						.opsForHash()
						.get(hashKey(companyId, distributorId), String.valueOf(itemId));
		if (raw == null) {
	...
	/** 写入 field→uri；不设 expire；field 为 distributorId（非 itemId） */
	public void hashSet(long companyId, long distributorId, String uri) {
		sharedStringRedisTemplate
				.opsForHash()
				.put(
						hashKey(companyId, distributorId),
						String.valueOf(distributorId),
						uri == null ? "" : uri);
	}

	private static String hashKey(long companyId, long distributorId) {
		return "hash:distributor_" + distributorId + "_item_qr_code:"
