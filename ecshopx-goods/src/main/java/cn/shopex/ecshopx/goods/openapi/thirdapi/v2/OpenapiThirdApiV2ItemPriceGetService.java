...
@Service
public class OpenapiThirdApiV2ItemPriceGetService {
	...
	public OpenapiThirdApiV2ItemPriceGetService(
	...
	public Map<String, Object> executeGetPriceDetail(long companyId, Map<String, Object> mergedParams) {
		try {
			...
			result.put("price", fenToYuanString(itemInfo.getPrice()));
			result.put("market_price", fenToYuanString(itemInfo.getMarketPrice()));
			result.put("cost_price", fenToYuanString(itemInfo.getCostPrice()));
	...
	private void appendDistributorBranch(
		...
		if (distributorItemOpt.isPresent()) {
			...
			Integer shopPriceFen = row.getPrice() != null ? row.getPrice().intValue() : null;
			result.put("price", fenToYuanString(shopPriceFen));
		}
	...
	private static String fenToYuanString(Integer fen) {
		int value = fen != null ? fen : 0;
