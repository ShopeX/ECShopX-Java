...
class ScheduleMerchantDailyStatisticHandlerTest {
	...
	@InjectMocks
	private ScheduleMerchantDailyStatisticHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleMerchantDailyStatisticHandler.class);
	...
	void execute_ok_noAlert() {
		handler.execute();
		verify(merchantDataService, times(1)).scheduleInitStatistic();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								&& e.getFormattedMessage() != null
								&& e.getFormattedMessage().contains("merchant-daily-statistic")
	...
	void execute_failure_publishesAlertAndRethrows() {
		...
		doThrow(err).when(merchantDataService).scheduleInitStatistic();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("merchant-daily-statistic");
		assertThat(listAppender.list)
				.anyMatch(
