...
@Component
public class OpenapiMemberOperateLogV2ListFilterBuilder {
	...
	public OpenapiMemberOperateLogV2ListFilterBuilder(MemberAccountService memberAccountService) {
	...
	public FilterSpec build(
		...
		if (startDatePresent && !phpEmpty(startDateRaw)) {
			createdGte = parseDateTimeToEpoch(startDateRaw);
		}

		Integer createdLte = null;
		if (endDatePresent && !phpEmpty(endDateRaw)) {
			createdLte = parseDateTimeToEpoch(endDateRaw);
		}

		return new FilterSpec(companyId, userId, createdGte, createdLte);
	}
