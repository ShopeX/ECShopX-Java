...
@Service
public class MemberArticleFavRemoveService {

	private final MemberArticleFavMapper memberArticleFavMapper;

	public MemberArticleFavRemoveService(MemberArticleFavMapper memberArticleFavMapper) {
		this.memberArticleFavMapper = memberArticleFavMapper;
	}

	public boolean deleteArticleFav(long companyId, long userId, long articleId) {
		if (companyId <= 0L || userId <= 0L || articleId == 0L) {
		...
		memberArticleFavMapper.delete(w);
		return true;
	}
}
