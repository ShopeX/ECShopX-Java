...
/**
 * 有数 {@code /data-api/v1/order/add_order_sum}；对齐 PHP {@code Analysis\Client::addOrderSum} + Kernel 签名。
 */
@Service
public class YoushuAnalysisAddOrderSumClient implements YoushuAnalysisAddOrderSumPort {

	private static final RestTemplate REST = new RestTemplate();

	private final ObjectMapper objectMapper;

	public YoushuAnalysisAddOrderSumClient(ObjectMapper objectMapper) {
		this.objectMapper = objectMapper;
	}

	@Override
	public void addOrderSum(
			String dataSourceId, YoushuOrderSumRow orderSum, YoushuOpenApiCredentials credentials) {
		...
		JsonNode rowNode = objectMapper.valueToTree(orderSum);
		...
		UriComponentsBuilder b = UriComponentsBuilder.fromUriString(base + "/data-api/v1/order/add_order_sum");
		...
		try {
			...
			if (rc != 0) {
				throw new RuntimeException(
						"有数 add_order_sum 失败: " + root.path("errmsg").asText("retcode=" + rc));
			}
