...
/**
 * 有数 analysis add_wxapp_visit_page；对齐 PHP {@code Analysis\Client::addWxappVisitPage} + Kernel 签名。
 */
@Service
public class YoushuAnalysisAddWxappVisitPageClient implements YoushuAnalysisAddWxappVisitPagePort {

	private static final RestTemplate REST = new RestTemplate();

	private final ObjectMapper objectMapper;

	public YoushuAnalysisAddWxappVisitPageClient(ObjectMapper objectMapper) {
	...
	public void addWxappVisitPage(
		...
		UriComponentsBuilder b =
				UriComponentsBuilder.fromUriString(base + "/data-api/v1/analysis/add_wxapp_visit_page");
		...
		try {
			...
			if (rc != 0) {
				throw new RuntimeException(
						"有数 add_wxapp_visit_page 失败: " + root.path("errmsg").asText("retcode=" + rc));
			}
