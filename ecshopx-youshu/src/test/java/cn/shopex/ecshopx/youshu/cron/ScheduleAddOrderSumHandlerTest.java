...
class ScheduleAddOrderSumHandlerTest {
	...
	@InjectMocks
	private ScheduleAddOrderSumHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleAddOrderSumHandler.class);
	...
	@Test
	@DisplayName("plan §1：handler 成功；§3 业务在 service；无 CronAlertEvent")
	void execute_ok_noAlert() {
		handler.execute();
		verify(youshuTaskService, times(1)).scheduleAddOrderSum();
	...
	@Test
	@DisplayName("plan §1：失败路径 CronAlertEvent + 重抛；§3 不区分业务分支")
	void execute_fail_alertAndRethrow() {
		RuntimeException err = new RuntimeException("test");
		doThrow(err).when(youshuTaskService).scheduleAddOrderSum();
