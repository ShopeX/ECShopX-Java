...
class YoushuNormalOrderCancelSrDataSyncServiceTest {
	...
	@InjectMocks
	private YoushuNormalOrderCancelSrDataSyncService service;

	@Test
	void whenNoSetting_thenNoPush() {
		when(youshuSettingMapper.selectOne(any(Wrapper.class))).thenReturn(null);

		service.syncOrderAfterNormalCancel(1L, 10L);
	...
	@Test
	void whenSetting_thenResolvesDataSourceAndPushesWithCancelFields() {
		...
		service.syncOrderAfterNormalCancel(1L, "10");
						...
						argThat(
								m ->
										Long.valueOf(10L).equals(m.get("order_id"))
												...
												&& "CANCEL".equals(m.get("order_status"))
												&& "SUCCESS".equals(m.get("cancel_status"))),
