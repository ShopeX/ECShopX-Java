...
class NormalOrderCancelYoushuDispatchListenerDispatchConsumerRuntimeTest {

	@Mock
	private YoushuNormalOrderCancelSrDataSyncService youshuNormalOrderCancelSrDataSyncService;

	private DispatchConsumerRuntime runtime;

	@BeforeEach
	void setUp() {
		NormalOrderCancelYoushuDispatchListener listener =
				new NormalOrderCancelYoushuDispatchListener(youshuNormalOrderCancelSrDataSyncService);
	...
	@Test
	@DisplayName("EVENT_NORMAL_ORDER_CANCEL: youshu cancel listener — DispatchConsumerRuntime consume")
	void consume_event_message_invokes_sync_service_once() {
		...
		DispatchMessage message =
				new DispatchMessage(
						...
						"trace-normal-order-cancel-youshu",
						OrdersDispatchEventNames.LISTENER_YOUSHU_ORDERS_NORMAL_ORDER_CANCEL);

		runtime.consume(message, 1);

		verify(youshuNormalOrderCancelSrDataSyncService, times(1)).syncOrderAfterNormalCancel(eq(companyId), eq(orderId));
	}
}
