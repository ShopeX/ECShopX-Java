...
class ScheduleAddWxappVisitDistributionHandlerTest {

	private static final String HANDLER_SHORT_NAME = "youshu-add-wxapp-visit-distr";
	...
	@InjectMocks
	private ScheduleAddWxappVisitDistributionHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleAddWxappVisitDistributionHandler.class);
	...
	@Test
	@DisplayName("§3 步骤 1–3 入口：execute 成功；service scheduleAddWxappVisitDistribution 无未捕获异常；无 CronAlertEvent")
	void execute_ok_logsInfoAndNoAlert() {
		handler.execute();
		verify(youshuTaskService, times(1)).scheduleAddWxappVisitDistribution();
	...
	@Test
	@DisplayName("§3 步骤 1–3 入口：execute 失败；CronAlertEvent + 重抛")
	void execute_failure_publishesAlertAndRethrows() {
		RuntimeException err = new RuntimeException("test");
		doThrow(err).when(youshuTaskService).scheduleAddWxappVisitDistribution();
