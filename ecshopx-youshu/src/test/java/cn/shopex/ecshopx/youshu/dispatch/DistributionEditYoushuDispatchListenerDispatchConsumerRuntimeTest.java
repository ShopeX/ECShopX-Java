...
class DistributionEditYoushuDispatchListenerDispatchConsumerRuntimeTest {

	@Mock
	private YoushuDistributionEditSrDataSyncService youshuDistributionEditSrDataSyncService;

	private DispatchConsumerRuntime runtime;

	@BeforeEach
	void setUp() {
		DistributionEditYoushuDispatchListener listener =
				new DistributionEditYoushuDispatchListener(youshuDistributionEditSrDataSyncService);
	...
	@Test
	@DisplayName("EVENT_DISTRIBUTION_EDIT: youshu listener — DispatchConsumerRuntime consume")
	void consume_event_message_invokes_sync_service_once() {
		...
		DispatchMessage message =
				new DispatchMessage(
						...
						"trace-distribution-edit-youshu",
						DistributionDispatchEventNames.LISTENER_YOUSHU_BUNDLE_DISTRIBUTION);

		runtime.consume(message, 1);

		verify(youshuDistributionEditSrDataSyncService, times(1)).syncAfterDistributionEdit(eq(entities));
	}
}
