...
public class WorkWechatReportNotifyService {
	...
	public ResponseEntity<?> handle(
			...
			String rawBody) {
		try {
			log.info("report notify start corpid={} query={}", corpid, queryParams);
			if (rawBody != null && log.isDebugEnabled()) {
				log.debug("report notify raw body length={}", rawBody.length());
			}
			...
			Object reportObj = agents.get("report");
			if (!(reportObj instanceof Map<?, ?> report)) {
				throw new ResourceException("企业微信配置无效");
			}
			String token = trimToEmpty(report.get("token"));
			String aesKey = trimToEmpty(report.get("aes_key"));
			...
			if (rawBody != null && !rawBody.isEmpty()) {
				...
				if (err == WXBizMsgCrypt.OK) {
					String xml = outXml[0];
					log.info("report notify decrypted xml={}", xml);
					Map<String, Object> eventMap = xmlParser.toMap(xml);
					log.info("report notify event map={}", eventMap);
					addressBookPort.handleAddressBookEvent(companyIdOrNull, eventMap);
					return ResponseEntity.ok().build();
				}
				log.info("report notify decrypt fail errCode={}", err);
				return ResponseEntity.ok().build();
			}
			return ResponseEntity.ok().build();
		} catch (Exception e) {
			...
			log.info("report notify end corpid={}", corpid);
		}
	}
