...
@Service
public class ThingsDetailService {

	private final ThingsMapper thingsMapper;

	public ThingsDetailService(ThingsMapper thingsMapper) {
		this.thingsMapper = thingsMapper;
	}

	public Map<String, Object> getDetail(long operatorCompanyId, long thingId) {
		Things entity = thingsMapper.selectById(thingId);
		if (entity == null) {
			throw new ResourceException("数据不存在");
		}
		Long cid = entity.getCompanyId();
		if (cid == null || cid.longValue() != operatorCompanyId) {
			throw new ResourceException("获取物品信息有误，请确认商品ID.", 422);
		}
