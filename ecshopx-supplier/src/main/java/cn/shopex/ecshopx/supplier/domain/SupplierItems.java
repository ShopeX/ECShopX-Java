 ...
 * 商品表
 *
 * <p>索引：ix_company_id、ix_default_item_id、ix_item_category、ix_item_bn、ix_goods_id、ix_supplier_id、ix_is_default、ix_is_medicine、ix_is_prescription、ix_default_item_list（company_id, default_item_id,
 * approve_status, item_type, item_category）等
 */
@Data
@MpTable(value = "supplier_items", comment = "商品表", indexes = {@MpIndex(name = "ix_company_id", columns = {"company_id"}), @MpIndex(name = "ix_default_item_id", columns = {"default_item_id"}), @MpIndex(name = "ix_item_category", columns = {"item_category"}), @MpIndex(name = "ix_item_bn", columns = {"item_bn"}), @MpIndex(name = "ix_goods_id", columns = {"goods_id"}), @MpIndex(name = "ix_supplier_id", columns = {"supplier_id"}), @MpIndex(name = "ix_is_default", columns = {"is_default"}), @MpIndex(name = "ix_is_medicine", columns = {"is_medicine"}), @MpIndex(name = "ix_is_prescription", columns = {"is_prescription"}), @MpIndex(name = "ix_default_item_list", columns = {"company_id", "default_item_id", "approve_status", "item_type", "item_category"})})
public class SupplierItems {
    ...
    /**
     * 审核状态 submiting待提交 approved成功 processing审核中 rejected审核拒绝
     */
    @MpField(value = "audit_status", columnType = "string", nullable = true, comment = "审核状态 submiting待提交 approved成功 processing审核中 rejected审核拒绝", defaultValue = "approved")
    ...
    /** SPU编码 */
    @MpField(value = "goods_bn", columnType = "string", length = 255, nullable = true, comment = "SPU编码")
