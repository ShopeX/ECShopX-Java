...
public final class OpenapiThirdApiV2OrderIncrListParams {

	private OpenapiThirdApiV2OrderIncrListParams() {}
	...
	public static PageSpec resolve(String pageParam, String pageSizeParam, Map<String, Object> body) {
		...
		validatePage(pageRaw, page);
		validatePageSize(pageSizeRaw, pageSize);

		return new PageSpec(page, pageSize);
	}

	private static void validatePage(String pageRaw, int page) {
		if (pageRaw != null && !pageRaw.isEmpty() && !isPhpInteger(pageRaw)) {
	...
	private static void validatePageSize(String pageSizeRaw, int pageSize) {
		if (pageSizeRaw != null && !pageSizeRaw.isEmpty() && !isPhpInteger(pageSizeRaw)) {
	...
	private static boolean isPhpInteger(String raw) {
		String t = raw.trim();
		if (t.isEmpty()) {
			return false;
		}
