...
@Service
public class OfflineBankAccountMultiLangWriteService {

	private static final String TABLE_NAME = "offline_bank_account";
	private static final String MODULE_NAME = "offline_bank_account";
	private static final List<String> LANG_FIELDS = List.of("bank_account_name", "bank_name", "remark");

	private final NamedParameterJdbcTemplate namedParameterJdbcTemplate;

	public OfflineBankAccountMultiLangWriteService(NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
		this.namedParameterJdbcTemplate = namedParameterJdbcTemplate;
	}

	public void syncAfterInsert(long dataId, Map<String, Object> createDataSnapshot, String requestLangTag) {
		...
		for (String field : LANG_FIELDS) {
			...
			p.addValue("moduleName", MODULE_NAME);
	...
	public void syncAfterUpdate(long dataId, int companyId, Map<String, Object> langSnapshot, String requestLangTag) {
		...
		for (String field : LANG_FIELDS) {
			if (!langSnapshot.containsKey(field)) {
				continue;
			}
			...
			up.addValue("moduleName", MODULE_NAME);
			...
			if (affected == 0) {
				...
				ip.addValue("moduleName", MODULE_NAME);
