...
public class DiscountGoodsUploadEspierHandler extends AbstractEspierTableUploadHandler {
	...
	public Optional<Map<String, Object>> trySyncProcess(
		...
		try {
			return Optional.of(EspierGoodsMarketingStyleSyncSupport.parseSync(
					file, getHeaderTitle(companyId), PriceMode.DISCOUNT, companyId, distributorId, itemsLookup));
		} catch (IOException e) {
	...
	protected void handleBusinessRow(EspierUploadRowContext ctx, Map<String, Object> row) {
		EspierImportRowSink sink = rowSinks.get("discount_goods");
		if (sink == null) {
			throw new IllegalStateException("missing EspierImportRowSink for discount_goods");
		}
		sink.acceptRow(
				ctx.companyId(),
				ctx.operatorId(),
