...
public class MarketingGoodsUploadEspierHandler extends AbstractEspierTableUploadHandler {
	...
	public Optional<Map<String, Object>> trySyncProcess(
		...
		try {
			return Optional.of(EspierGoodsMarketingStyleSyncSupport.parseSync(
					file, getHeaderTitle(companyId), PriceMode.MARKETING, companyId, distributorId, itemsLookup));
		} catch (IOException e) {
	...
	protected void handleBusinessRow(EspierUploadRowContext ctx, Map<String, Object> row) {
		EspierImportRowSink sink = rowSinks.get("marketing_goods");
		if (sink == null) {
			throw new IllegalStateException("missing EspierImportRowSink for marketing_goods");
		}
		sink.acceptRow(
				ctx.companyId(),
				ctx.operatorId(),
