...
@Service
public class AftersalesRefundRecordExportFileJobHandler {

	public static final String EXPORT_FILE_JOB_TYPE_REFUND_RECORD_COUNT = "refund_record_count";

	private static final Logger log = LoggerFactory.getLogger(AftersalesRefundRecordExportFileJobHandler.class);

	private final AftersalesRefundListService aftersalesRefundListService;
	private final AftersalesRefundRecordCsvExportService csvExportService;
	private final ExportLogCreateService exportLogCreateService;
	private final OperatorsMapper operatorsMapper;

	public AftersalesRefundRecordExportFileJobHandler(
			AftersalesRefundListService aftersalesRefundListService,
			AftersalesRefundRecordCsvExportService csvExportService,
	...
	public void run(AftersalesRefundRecordExportJobContext ctx) {
		...
		long jobCount = aftersalesRefundListService.countByFilterMapForRefundLogExport(work, ctx.companyId());
		if (jobCount <= 0L) {
		...
		exportLogCreateService.createFinishLog(
				...
				merchantIdForLog,
				supplierIdForLog,
				EXPORT_FILE_JOB_TYPE_REFUND_RECORD_COUNT,
