...
class ScheduleRefundHandlerTest {
	...
	@InjectMocks
	private ScheduleRefundHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleRefundHandler.class);
	...
	@Test
	@DisplayName("正常：dispatched=3 且不发布 CronAlertEvent")
	void execute_ok() {
		when(aftersalesRefundService.scheduleRefund()).thenReturn(3);
		handler.execute();
		verify(aftersalesRefundService, times(1)).scheduleRefund();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								&& e.getFormattedMessage() != null
								&& e.getFormattedMessage().contains("schedule-refund")
	...
	void execute_fails() {
		...
		doThrow(err).when(aftersalesRefundService).scheduleRefund();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("schedule-refund");
		assertThat(listAppender.list)
				.anyMatch(
