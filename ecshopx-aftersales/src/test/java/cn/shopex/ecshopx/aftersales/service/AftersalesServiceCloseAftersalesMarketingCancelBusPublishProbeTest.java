...
class AftersalesServiceCloseAftersalesMarketingCancelBusPublishProbeTest {
	...
	@Test
	@DisplayName("closeAftersales afterCommit: trade aftersales cancel async fan-out publish after TradeAftersalesCancelSpringEvent")
	void closeAftersales_afterCommit_invokesTradeAftersalesCancelFanOutPublisher() {
		...
		when(aftersalesRefundService.updateRefundByAftersalesKeys(eq(COMPANY_ID), eq(AFTERSALES_BN), any())).thenReturn(1);
		...
		inOrder.verify(tradeAftersalesCancelDispatchPublisher).publish(marketingCap.capture());
		inOrder.verify(thirdPartySaasErpCancelPublisher).publish(any());
		inOrder.verify(jushuitanPublisher).publish(any());
		...
		Map<String, Object> m = marketingCap.getValue();
		assertEquals(COMPANY_ID, ((Number) m.get("company_id")).longValue());
		assertEquals(ORDER_ID, ((Number) m.get("order_id")).longValue());
		assertEquals(AFTERSALES_BN, ((Number) m.get("aftersales_bn")).longValue());
	}
}
