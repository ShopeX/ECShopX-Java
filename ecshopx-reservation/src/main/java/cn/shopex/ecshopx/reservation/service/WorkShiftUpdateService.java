...
public class WorkShiftUpdateService {
	...
	public Map<String, Object> updateWorkShift(
			long companyId,
			long shopId,
		...
		WorkShift existing =
				workShiftMapper.selectOne(
						new LambdaQueryWrapper<WorkShift>()
								.eq(WorkShift::getId, workShiftId)
								.eq(WorkShift::getCompanyId, companyId)
								.eq(WorkShift::getShopId, shopId)
								.eq(WorkShift::getResourceLevelId, resourceLevelId)
								.eq(WorkShift::getWorkDate, workDateStartOfDayEpoch));
		if (existing == null) {
			throw new ResourceException("排班不存在");
		}

		existing.setShiftTypeId(shiftTypeIdLong);
		existing.setUpdated((int) Instant.now().getEpochSecond());
		int rows = workShiftMapper.updateById(existing);
		if (rows == 0) {
			throw new ResourceException("排班不存在");
		}
