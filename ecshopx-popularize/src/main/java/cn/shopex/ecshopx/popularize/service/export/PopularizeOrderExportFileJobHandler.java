...
@Service
public class PopularizeOrderExportFileJobHandler {

	private static final Logger log = LoggerFactory.getLogger(PopularizeOrderExportFileJobHandler.class);

	private final OperatorsQueryService operatorsQueryService;
	private final PopularizeOrderCsvExportService popularizeOrderCsvExportService;
	private final ExportLogCreateService exportLogCreateService;

	public PopularizeOrderExportFileJobHandler(
			OperatorsQueryService operatorsQueryService,
			PopularizeOrderCsvExportService popularizeOrderCsvExportService,
			ExportLogCreateService exportLogCreateService) {
		this.operatorsQueryService = operatorsQueryService;
		this.popularizeOrderCsvExportService = popularizeOrderCsvExportService;
		this.exportLogCreateService = exportLogCreateService;
	}

	public void run(PopularizeOrderExportJobContext ctx) {
		long supplierId = 0L;
		...
		if (log.isTraceEnabled() && supplierId > 0L) {
			log.trace("popularize order export ignores supplier_id filter even when supplierId={}", supplierId);
		}
		...
		Optional<Map<String, String>> upload =
				popularizeOrderCsvExportService.runExport(merged, ctx.datapassBlock());
		if (upload.isEmpty()) {
