...
@Service
public class PromoterExportFileJobHandler {

	private static final Logger log = LoggerFactory.getLogger(PromoterExportFileJobHandler.class);

	private final OperatorsQueryService operatorsQueryService;
	private final PopularizePromoterCsvExportService popularizePromoterCsvExportService;
	private final ExportLogCreateService exportLogCreateService;

	public PromoterExportFileJobHandler(
			OperatorsQueryService operatorsQueryService,
			PopularizePromoterCsvExportService popularizePromoterCsvExportService,
			ExportLogCreateService exportLogCreateService) {
		this.operatorsQueryService = operatorsQueryService;
		this.popularizePromoterCsvExportService = popularizePromoterCsvExportService;
		this.exportLogCreateService = exportLogCreateService;
	}

	public void run(PromoterExportJobContext ctx) {
		long supplierId = 0L;
		...
		if (log.isTraceEnabled() && supplierId > 0L) {
			log.trace("popularize export ignores supplier_id filter even when supplierId={}", supplierId);
		}
		...
		Optional<Map<String, String>> upload =
				popularizePromoterCsvExportService.runExport(merged, ctx.datapassBlock());
		if (upload.isEmpty()) {
