...
@Service
public class PromoterDisabledService {

	private final PromoterMapper promoterMapper;

	public PromoterDisabledService(PromoterMapper promoterMapper) {
		this.promoterMapper = promoterMapper;
	}

	@Transactional(rollbackFor = Exception.class)
	public void updatePromoterDisabled(long companyId, String userIdRaw, String activeRaw) {
		...
		String a = activeRaw == null ? "true" : activeRaw.trim();
		int disabledColumn = "true".equals(a) ? 0 : 1;
		...
		int rows = promoterMapper.update(
				...
				new LambdaUpdateWrapper<Promoter>()
						...
						.set(Promoter::getDisabled, disabledColumn));
		if (rows != 1) {
