...
@Component
public class WxopenTemplateSendJobHandler implements DispatchHandler {

	private static final Logger log = LoggerFactory.getLogger(WxopenTemplateSendJobHandler.class);

	private final WxaTemplateMsgActivityRemindSendService wxaTemplateMsgActivityRemindSendService;

	public WxopenTemplateSendJobHandler(WxaTemplateMsgActivityRemindSendService wxaTemplateMsgActivityRemindSendService) {
	...
	public void handle(Map<String, Object> payload) {
		try {
			...
			wxaTemplateMsgActivityRemindSendService.send(biz, forceFire);
		} catch (RuntimeException e) {
			log.debug("WxopenTemplateSendJob failed: {}", e.toString());
		}
	}
}
