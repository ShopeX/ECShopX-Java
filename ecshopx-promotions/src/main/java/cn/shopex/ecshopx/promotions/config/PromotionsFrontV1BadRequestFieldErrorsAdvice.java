...
/**
 * Dingo-style {@link BadRequestException} for promotions front v1: when the handler declares
 * {@code badRequest != NONE}, builds a JSON body with {@code data.message} and {@code data.status_code}
 * (and {@code data.errors} when field errors are present). When there are no field errors and no
 * embedded status code, still returns that two-key {@code data} shape for {@code badRequest != NONE};
 * only when {@code badRequest == NONE} or no {@link DingoResponse} is resolved does this advice
 * defer to other handlers.
 */
@RestControllerAdvice(basePackages = "cn.shopex.ecshopx.promotions.api.front.v1")
@Order(Ordered.HIGHEST_PRECEDENCE)
public class PromotionsFrontV1BadRequestFieldErrorsAdvice {
