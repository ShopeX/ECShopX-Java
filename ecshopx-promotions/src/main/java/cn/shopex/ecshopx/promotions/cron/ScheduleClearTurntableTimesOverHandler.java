...
public class ScheduleClearTurntableTimesOverHandler {

	private static final String HANDLER_SHORT = "clear-turntable-times-over";

	private final TurntableScheduleService turntableScheduleService;
	...
	public void execute() {
		...
		try {
			ScheduleClearResult result = turntableScheduleService.scheduleClearTurntableTimesOver();
			log.info(
					"[cron][{}] done, cost={}ms, redisDeleted={}",
					HANDLER_SHORT,
					System.currentTimeMillis() - start,
					result.redisKeysDeleted());
		} catch (Exception e) {
			log.error(
					"[cron][{}] failed, cost={}ms",
					HANDLER_SHORT,
