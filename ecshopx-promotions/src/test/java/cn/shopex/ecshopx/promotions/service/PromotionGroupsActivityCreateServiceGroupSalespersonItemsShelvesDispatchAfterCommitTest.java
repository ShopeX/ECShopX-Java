...
class PromotionGroupsActivityCreateServiceGroupSalespersonItemsShelvesDispatchAfterCommitTest {
	...
	@Test
	void createPromotionGroupsActivity_whenTransactionCommits_invokesEnqueueGroupSalespersonItemsShelvesOnceAfterCommit_withGroupActivityType() {
		...
		when(activityMapper.selectCount(isA(com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper.class)))
				.thenReturn(0L);

		JdbcTemplate jdbcTemplate = mock(JdbcTemplate.class);
		when(jdbcTemplate.queryForObject(contains("COUNT(1)"), eq(Long.class), any(), any())).thenReturn(0L);

		PromotionGroupsActivityGroupParamValidationService validationService =
				new PromotionGroupsActivityGroupParamValidationService(messageSource, catalogAccess, activityMapper, jdbcTemplate);

		PromotionGroupsActivityMultiLangWriteService multiLang = mock(PromotionGroupsActivityMultiLangWriteService.class);
		doNothing()
				.when(multiLang)
		...
		MarketingActivityPostCommitJobsService marketingActivityPostCommitJobsService =
				mock(MarketingActivityPostCommitJobsService.class);
		...
		when(relGoodsWriteService.normalizeAndValidateItems(anyMap(), eq(COMPANY_ID), any(Locale.class)))
				.thenReturn(List.of(new NormalizedGroupSkuItem(ITEM_ID, 999L, 20L, "title", "pic", "spec")));
		doNothing()
		...
		PromotionGroupsActivityCreateService svc =
				new PromotionGroupsActivityCreateService(
						...
						validationService,
						tagPublisher,
						marketingActivityPostCommitJobsService,
		...
		tt.executeWithoutResult(
				st -> {
					...
					verify(relGoodsWriteService)
							.replaceRelGoods(eq(GROUPS_ACTIVITY_ID), eq(COMPANY_ID), anyList(), anyInt());
					verifyNoInteractions(marketingActivityPostCommitJobsService);
				});

		verify(marketingActivityPostCommitJobsService)
				.enqueueGroupSalespersonItemsShelves(eq(COMPANY_ID), eq(GROUPS_ACTIVITY_ID));
		verifyNoMoreInteractions(marketingActivityPostCommitJobsService);
	}
