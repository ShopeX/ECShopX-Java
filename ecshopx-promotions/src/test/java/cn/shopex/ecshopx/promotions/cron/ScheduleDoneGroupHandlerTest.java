...
class ScheduleDoneGroupHandlerTest {
	...
	@InjectMocks
	private ScheduleDoneGroupHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleDoneGroupHandler.class);
	...
	@Test
	@DisplayName("plan §5 handler: 成功路径；无 CronAlertEvent（analysis 步骤 2 非 0 由 service 覆盖）")
	void execute_ok_noAlert() {
		when(promotionGroupsTeamService.scheduleAutoDoneGroup()).thenReturn(3);
		handler.execute();
		verify(promotionGroupsTeamService, times(1)).scheduleAutoDoneGroup();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								&& e.getFormattedMessage() != null
								&& e.getFormattedMessage().contains("done-group")
								&& e.getFormattedMessage().contains("done")
								&& e.getFormattedMessage().contains("processed=3"));
	}

	@Test
	@DisplayName("plan §5 handler: 异常路径 + CronAlertEvent + 重抛；ERROR 与 INFO 对称含 cost=…ms")
	void execute_failure_publishesAlertAndRethrows() {
		RuntimeException err = new RuntimeException("test error");
		doThrow(err).when(promotionGroupsTeamService).scheduleAutoDoneGroup();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("done-group");
		assertThat(listAppender.list)
				.anyMatch(
