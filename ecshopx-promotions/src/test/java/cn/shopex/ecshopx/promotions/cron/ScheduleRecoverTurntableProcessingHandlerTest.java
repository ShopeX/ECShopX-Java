...
class ScheduleRecoverTurntableProcessingHandlerTest {

	private static final String SHORT = "recover-turntable-processing";

	@Mock private TurntableDrawRecoverService turntableDrawRecoverService;
	@Mock private ApplicationEventPublisher eventPublisher;

	@InjectMocks private ScheduleRecoverTurntableProcessingHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleRecoverTurntableProcessingHandler.class);
	...
	void execute_ok_noAlert() {
		when(turntableDrawRecoverService.recoverStuck(60)).thenReturn(3);
		handler.execute();
		verify(turntableDrawRecoverService, times(1)).recoverStuck(60);
		...
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								...
								&& e.getFormattedMessage().contains("recovered=3"));
	}

	@Test
	void execute_failure_publishesAlertAndRethrows() {
		RuntimeException err = new RuntimeException("cron test");
		doThrow(err).when(turntableDrawRecoverService).recoverStuck(60);
