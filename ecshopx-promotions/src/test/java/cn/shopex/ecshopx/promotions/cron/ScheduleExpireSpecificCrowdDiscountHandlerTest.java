...
import cn.shopex.ecshopx.promotions.service.SpecificCrowdDiscountService;
...
class ScheduleExpireSpecificCrowdDiscountHandlerTest {
	...
	@InjectMocks
	private ScheduleExpireSpecificCrowdDiscountHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleExpireSpecificCrowdDiscountHandler.class);
	...
	void execute_ok_noAlert() {
		when(specificCrowdDiscountService.scheduleExpiredPromotion()).thenReturn(3);
		handler.execute();
		verify(specificCrowdDiscountService, times(1)).scheduleExpiredPromotion();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								&& e.getFormattedMessage() != null
								&& e.getFormattedMessage().contains("expire-specific-crowd-discount")
	...
	void execute_failure_publishesAlertAndRethrows() {
		...
		doThrow(err).when(specificCrowdDiscountService).scheduleExpiredPromotion();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("expire-specific-crowd-discount");
