...
class ScheduleExpiredPromotionMonthHandlerTest {
	...
	@InjectMocks
	private ScheduleExpiredPromotionMonthHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleExpiredPromotionMonthHandler.class);
	...
	void execute_ok_noAlert() {
		when(specificCrowdDiscountService.scheduleExpiredPromotionMonth()).thenReturn(3);
		handler.execute();
		verify(specificCrowdDiscountService, times(1)).scheduleExpiredPromotionMonth();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								&& e.getFormattedMessage() != null
								&& e.getFormattedMessage().contains("expire-crowd-month")
	...
	void execute_failure_publishesAlertAndRethrows() {
		...
		doThrow(err).when(specificCrowdDiscountService).scheduleExpiredPromotionMonth();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("expire-crowd-month");
