...
import cn.shopex.ecshopx.common.dispatch.SendTaskProgressNoticeJobDispatchPublisher;
...
@Service
public class SalespersonTaskShareService {
	...
	private final SendTaskProgressNoticeJobDispatchPublisher sendTaskProgressNoticeJobDispatchPublisher;

	public SalespersonTaskShareService(ShopsRelSalespersonMapper shopsRelSalespersonMapper,
			...
			SendTaskProgressNoticeJobDispatchPublisher sendTaskProgressNoticeJobDispatchPublisher) {
		...
		this.sendTaskProgressNoticeJobDispatchPublisher = sendTaskProgressNoticeJobDispatchPublisher;
	}

	@Transactional(rollbackFor = Exception.class)
	public boolean share(long companyId, long userId, String username, long salespersonId, String type, String id) {
		if (salespersonId <= 0L) {
			return false;
		}
		...
		SalespersonTask task = salespersonTaskMapper.selectActiveTaskForDistributor(
				companyId, distributorId, TASK_TYPE_SHARE, now);
		...
		final long sid = salespersonId;
		final String uname = username;
		TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
			@Override
			public void afterCommit() {
				sendTaskProgressNoticeJobDispatchPublisher.publish(cid, tid, sid, uname);
			}
		});
		return true;
	}
