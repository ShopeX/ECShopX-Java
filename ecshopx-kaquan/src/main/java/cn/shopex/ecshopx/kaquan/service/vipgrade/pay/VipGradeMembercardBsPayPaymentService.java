...
import cn.shopex.ecshopx.payment.service.membercard.MembercardBsPayPaymentSdkService.BsPayMembercardOutcome;
...
@Service
public class VipGradeMembercardBsPayPaymentService {

	private final StringRedisTemplate companysRedisTemplate;
	private final ObjectMapper objectMapper;
	private final VipGradeMembercardTradeSupportService tradeSupportService;
	private final MembercardBsPayPaymentSdkService bsPaySdkService;
	private final TradeMapper tradeMapper;

	public VipGradeMembercardBsPayPaymentService(
			@Qualifier("companysRedisTemplate") StringRedisTemplate companysRedisTemplate,
			ObjectMapper objectMapper,
			VipGradeMembercardTradeSupportService tradeSupportService,
			MembercardBsPayPaymentSdkService bsPaySdkService,
			TradeMapper tradeMapper) {
		this.companysRedisTemplate = companysRedisTemplate;
		this.objectMapper = objectMapper;
		this.tradeSupportService = tradeSupportService;
		this.bsPaySdkService = bsPaySdkService;
	...
	public Map<String, Object> pay(
		...
		Trade cached = tradeSupportService.findExistingWithPaymentParams(companyId, orderId, "bspay", payChannel);
		if (cached != null) {
			return mergeTradeInfo(parsePaymentParams(cached.getPaymentParams()), orderId, cached.getTradeId());
		}

		Map<String, Object> cfg = loadBsPaySettingOrThrow(companyId);

		Trade open = tradeSupportService.findOpenMembercardTrade(companyId, orderId, "bspay", payChannel);
		Trade trade = open != null ? open : tradeSupportService.createMembercardTrade(data, "bspay", payChannel, null);
		...
		String goodsDesc = StringUtils.hasText(detail) ? detail : body;
		String remark = stringVal(data.get("trade_source_type"));
		if (!StringUtils.hasText(remark)) {
			remark = "membercard";
		}
		String wxa = StringUtils.hasText(wxaAppId) ? wxaAppId : stringVal(data.get("wxa_appid"));
		String clientIp = stringVal(data.get("client_ip"));
		String source = stringVal(data.get("source"));

		BsPayMembercardOutcome outcome =
				bsPaySdkService.createPayment(
						companyId,
						cfg,
						trade.getTradeId(),
						orderId,
		...
		tradeMapper.update(
				...
				Wrappers.<Trade>lambdaUpdate()
						...
						.set(Trade::getBspayDivStatus, "NOTDIV")
	...
	private Map<String, Object> loadBsPaySettingOrThrow(long companyId) {
