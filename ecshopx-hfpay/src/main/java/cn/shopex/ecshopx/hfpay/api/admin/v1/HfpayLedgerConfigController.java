...
import cn.shopex.ecshopx.hfpay.service.ledger.HfpayLedgerConfigSaveService;
...
@AdminAuth
@ShopLog
@RestController("hfpayLedgerConfigAdminV1")
@RequestMapping("/api/v1")
public class HfpayLedgerConfigController {

	private final HfpayHfFileRequestMergeService hfpayHfFileRequestMergeService;
	private final HfpayLedgerConfigSaveService hfpayLedgerConfigSaveService;

	public HfpayLedgerConfigController(
			HfpayHfFileRequestMergeService hfpayHfFileRequestMergeService,
			HfpayLedgerConfigSaveService hfpayLedgerConfigSaveService) {
		this.hfpayHfFileRequestMergeService = hfpayHfFileRequestMergeService;
		this.hfpayLedgerConfigSaveService = hfpayLedgerConfigSaveService;
	}

	@Activated(routeAlias = "hfpay.ledgerconfig.index")
	@GetMapping(value = "/hfpay/ledgerconfig/index", name = "汇付获取分账配置")
	public ResponseEntity<ApiResult<Object>> index(HttpServletRequest request) {
		...
		Map<String, Object> map = hfpayLedgerConfigSaveService.getLedgerConfigForIndex(companyId);
		if (map == null) {
	...
	@Activated(routeAlias = "hfpay.ledgerconfig.save")
	@PostMapping(value = "/hfpay/ledgerconfig/save", name = "保存分账配置")
	public ResponseEntity<ApiResult<Map<String, Object>>> save(
		...
		Map<String, Object> data = hfpayLedgerConfigSaveService.save(companyId, merged);
