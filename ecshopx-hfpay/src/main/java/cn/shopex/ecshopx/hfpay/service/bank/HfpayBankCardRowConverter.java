...
public final class HfpayBankCardRowConverter {

	private static final DateTimeFormatter HFPAY_ADMIN_TS = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");

	private HfpayBankCardRowConverter() {}

	public static Map<String, Object> toDetailMap(HfpayBankCard e) {
		LinkedHashMap<String, Object> map = new LinkedHashMap<>();
		map.put("hfpay_bank_card_id", e.getHfpayBankCardId());
		...
		map.put("card_type", e.getCardType());
		map.put("bank_id", e.getBankId());
		map.put("bank_name", e.getBankName());
		map.put("card_num", e.getCardNum());
		map.put("bind_card_id", e.getBindCardId());
		map.put("is_cash", e.getIsCash());
