...
class ScheduleRetryAdaPayPaymentConfirmHandlerTest {
	...
	@InjectMocks
	private ScheduleRetryAdaPayPaymentConfirmHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleRetryAdaPayPaymentConfirmHandler.class);
	...
	void execute_ok() {
		when(adaPaymentService.adaPayPaymentConfirmRetry()).thenReturn(2);
		handler.execute();
		verify(adaPaymentService, times(1)).adaPayPaymentConfirmRetry();
		verify(eventPublisher, never()).publishEvent(any());
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								&& e.getFormattedMessage() != null
								&& e.getFormattedMessage().contains("adapay-confirm-retry")
	...
	void execute_fails() {
		...
		doThrow(err).when(adaPaymentService).adaPayPaymentConfirmRetry();
		...
		assertThat(cap.getValue().getHandlerShortName()).isEqualTo("adapay-confirm-retry");
		assertThat(listAppender.list)
				.anyMatch(
