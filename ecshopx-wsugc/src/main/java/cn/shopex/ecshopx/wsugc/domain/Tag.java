...
/** 图片tag */
@Data
@MpTable(value = "wsugc_tag", comment = "图片tag", indexes = {@MpIndex(name = "idx_user_id", columns = {"user_id"}), @MpIndex(name = "idx_company_id", columns = {"company_id"}), @MpIndex(name = "idx_mobile", columns = {"mobile"})})
public class Tag {

    @MpId(value = "tag_id", type = IdType.AUTO, columnType = "bigint")
    private Long tagId;

    /** 标签名称 */
    @MpField(value = "tag_name", columnType = "string", length = 250, comment = "标签名称")
