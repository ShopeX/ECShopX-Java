...
class NormalOrderPaySuccessWsugcDispatchListenerDispatchConsumerRuntimeTest {

	@Mock
	private WsugcNormalOrderPaySuccessApplyService wsugcNormalOrderPaySuccessApplyService;

	private DispatchConsumerRuntime runtime;

	@BeforeEach
	void setUp() {
		NormalOrderPaySuccessWsugcDispatchListener listener =
				new NormalOrderPaySuccessWsugcDispatchListener(wsugcNormalOrderPaySuccessApplyService);
	...
	@Test
	@DisplayName("EVENT_NORMAL_ORDER_PAY_SUCCESS: wsugc listener — DispatchConsumerRuntime consume")
	void consume_event_message_invokes_apply_service_once() {
		long companyId = 7L;
		long orderId = 99L;
		...
		DispatchMessage message =
				new DispatchMessage(
						...
						"trace-normal-order-pay-success-wsugc",
						OrdersDispatchEventNames.LISTENER_WSUGC_BUNDLE_ORDERS_NORMAL_ORDER_PAY_SUCCESS);

		runtime.consume(message, 1);

		verify(wsugcNormalOrderPaySuccessApplyService, times(1))
				.applyAfterNormalOrderPaySuccess(eq(companyId), eq(orderId));
	}
}
