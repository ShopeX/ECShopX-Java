...
import com.aliyun.dysmsapi20170525.models.CreateSmsSignResponseBody;
...
@Component
public class AliyunsmsCreateSmsSignClient {

	private static final Logger log = LoggerFactory.getLogger(AliyunsmsCreateSmsSignClient.class);

	private final AccessKeyMapper accessKeyMapper;

	public AliyunsmsCreateSmsSignClient(AccessKeyMapper accessKeyMapper) {
		this.accessKeyMapper = accessKeyMapper;
	}

	public void createSmsSign(
			long companyId,
			String signName,
			int signSource,
			String remark,
		...
		try {
			...
			CreateSmsSignRequest req = new CreateSmsSignRequest();
			...
			if (log.isDebugEnabled()) {
				log.debug(
						"CreateSmsSign companyId={} signName={} signSource={} thirdParty={} qualificationId={}",
						companyId,
						signName,
						signSource,
						thirdParty,
			...
			CreateSmsSignResponse resp = client.createSmsSign(req);
			CreateSmsSignResponseBody body = resp != null ? resp.getBody() : null;
			String code = body != null ? body.getCode() : null;
			if (log.isDebugEnabled()) {
				log.debug("CreateSmsSign response code={} message={}", code, body != null ? body.getMessage() : null);
			}
			if (body == null || !"OK".equals(code)) {
				String msg =
						body != null && body.getMessage() != null && !body.getMessage().isEmpty()
								? body.getMessage()
								: "添加阿里云短信签名失败";
				log.warn("CreateSmsSign not OK: {}", msg);
				throw new ResourceException(msg);
			}
		} catch (ResourceException e) {
			throw e;
		} catch (Exception e) {
			log.warn("CreateSmsSign SDK error: {}", e.getMessage());
			String fallback = "添加阿里云短信签名失败";
