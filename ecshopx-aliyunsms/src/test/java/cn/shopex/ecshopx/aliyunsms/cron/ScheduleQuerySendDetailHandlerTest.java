...
class ScheduleQuerySendDetailHandlerTest {

	private static final String SHORT = "aliyunsms-query-sms-send-detail";
	...
	@InjectMocks
	private ScheduleQuerySendDetailHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleQuerySendDetailHandler.class);
	...
	@Test
	@DisplayName("成功：不发布 CronAlertEvent，打 done 与 dbUpdated")
	void execute_ok() {
		when(smsService.scheduleQuerySendDetail()).thenReturn(3);
		handler.execute();
		verify(smsService, times(1)).scheduleQuerySendDetail();
		...
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								...
								&& e.getFormattedMessage().contains("dbUpdated=3"));
	}
	...
	void execute_fails() {
		...
		doThrow(err).when(smsService).scheduleQuerySendDetail();
