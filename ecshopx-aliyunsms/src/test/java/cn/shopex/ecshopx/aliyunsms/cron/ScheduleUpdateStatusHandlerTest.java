...
class ScheduleUpdateStatusHandlerTest {

	private static final String SHORT = "aliyunsms-update-task";
	...
	@InjectMocks
	private ScheduleUpdateStatusHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleUpdateStatusHandler.class);
	...
	@Test
	@DisplayName("成功：不发布 CronAlertEvent，打 done 与 taskRowsUpdated")
	void execute_ok() {
		when(taskService.scheduleUpdateStatus()).thenReturn(2);
		handler.execute();
		verify(taskService, times(1)).scheduleUpdateStatus();
		...
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								...
								&& e.getFormattedMessage().contains("taskRowsUpdated=2"));
	}
	...
	void execute_fails() {
		...
		doThrow(err).when(taskService).scheduleUpdateStatus();
