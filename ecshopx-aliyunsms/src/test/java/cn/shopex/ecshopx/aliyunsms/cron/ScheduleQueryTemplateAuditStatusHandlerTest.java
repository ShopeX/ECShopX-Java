...
class ScheduleQueryTemplateAuditStatusHandlerTest {

	private static final String SHORT = "aliyunsms-query-template-audit-status";
	...
	@InjectMocks
	private ScheduleQueryTemplateAuditStatusHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleQueryTemplateAuditStatusHandler.class);
	...
	@Test
	@DisplayName("成功：不发布 CronAlertEvent，打 done 与 jobsDispatched")
	void execute_ok() {
		when(templateService.scheduleQueryTemplateAuditStatus()).thenReturn(3);
		handler.execute();
		verify(templateService, times(1)).scheduleQueryTemplateAuditStatus();
		...
		assertThat(listAppender.list)
				.anyMatch(
						e -> e.getLevel() == Level.INFO
								...
								&& e.getFormattedMessage().contains("jobsDispatched=3"));
	}
	...
	void execute_fails() {
		...
		doThrow(err).when(templateService).scheduleQueryTemplateAuditStatus();
