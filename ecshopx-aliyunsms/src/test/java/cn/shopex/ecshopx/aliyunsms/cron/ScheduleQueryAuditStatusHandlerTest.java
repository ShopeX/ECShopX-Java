...
class ScheduleQueryAuditStatusHandlerTest {

	private static final String SHORT = "aliyunsms-query-sign-audit-status";
	...
	@InjectMocks
	private ScheduleQueryAuditStatusHandler handler;
	...
	void attachLogAppender() {
		...
		handlerLogger = (Logger) LoggerFactory.getLogger(ScheduleQueryAuditStatusHandler.class);
	...
	void execute_ok() {
		when(signService.scheduleQueryAuditStatus()).thenReturn(3);
		handler.execute();
		verify(signService, times(1)).scheduleQueryAuditStatus();
	...
	void execute_fails() {
		...
		doThrow(err).when(signService).scheduleQueryAuditStatus();
