...
@Service("dadaLocalDeliveryConfirmGoodsHttp")
@ConditionalOnProperty(prefix = "ecshopx.thirdparty.dada", name = "http-enabled", havingValue = "true")
public class DadaLocalDeliveryConfirmGoodsHttpService implements DadaLocalDeliveryConfirmGoodsPort {
	...
	public DadaLocalDeliveryConfirmGoodsHttpService(
	...
	@Override
	public void confirmGoods(long companyId, String mallOrderId) {
		if (mallOrderId == null || mallOrderId.trim().isEmpty()) {
			throw new ResourceException("订单号缺失");
		}
		String sourceId = resolveSourceIdForRequest(companyId);
		String inner;
		try {
			inner = objectMapper.writeValueAsString(Map.of("order_id", mallOrderId.trim()));
		} catch (Exception e) {
			throw new ResourceException("达达确认退回参数构造失败");
		}
		JsonNode root;
		try {
			root =
					dadaOpenPlatformJsonApiClient.postJsonBody(
							properties, sourceId, "/api/order/confirm/goods", inner);
		} catch (ResourceException e) {
			throw e;
		} catch (Exception e) {
			throw new ResourceException("达达确认退回请求异常: " + e.getMessage());
		}
