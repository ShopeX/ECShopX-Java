...
@AdminAuth
@ShopLog
@RestController("thirdPartyDmCrmAdminV1")
@RequestMapping("/api/v1")
public class DmCrmController {
	...
	public DmCrmController(DmCrmSettingAdminService dmCrmSettingAdminService) {
		this.dmCrmSettingAdminService = dmCrmSettingAdminService;
	}

	@Activated(routeAlias = "third.dmcrm.setting.set")
	@PostMapping(value = "/third/dmcrm/setting", name = "达摩CRM配置信息保存")
	public ResponseEntity<Map<String, Object>> setSetting(
		...
		dmCrmSettingAdminService.setSetting(companyId, merged);
		return ResponseEntity.ok(Map.of("data", Map.of("status", Boolean.TRUE)));
	}

	@Activated(routeAlias = "third.dmcrm.setting.get")
	@GetMapping(value = "/third/dmcrm/setting", name = "获取达摩CRM配置信息保存")
	public ResponseEntity<Map<String, Object>> getSetting(HttpServletRequest request) {
		...
		long companyId = readRequiredLong(ud, "company_id");
		Object data = dmCrmSettingAdminService.getSetting(companyId);
