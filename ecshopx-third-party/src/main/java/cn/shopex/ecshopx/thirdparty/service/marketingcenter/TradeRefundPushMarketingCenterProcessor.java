...
@Component
public class TradeRefundPushMarketingCenterProcessor {
	...
	public TradeRefundPushMarketingCenterProcessor(
	...
	public void handle(Map<String, Object> refundPayload) {
		if (refundPayload == null || refundPayload.isEmpty()) {
			return;
		}
		long companyId = longVal(refundPayload.get("company_id"));
		long orderId = longVal(refundPayload.get("order_id"));
		if (companyId <= 0L || orderId <= 0L) {
			return;
		}

		Object bnRaw = refundPayload.get("aftersales_bn");
		if (!hasMeaningfulAftersalesBn(bnRaw)) {
			...
			Map<String, Object> params = mergeTopLevel(refundPayload, orderInfo);
		...
		Map<String, Object> params = mergeForAftersalesPath(refundPayload, asInfo, orderInfo);
	...
	private static Map<String, Object> mergeForAftersalesPath(
			Map<String, Object> refund, Map<String, Object> asRow, Map<String, Object> order) {
		Map<String, Object> m = new LinkedHashMap<>(refund);
		m.putAll(order);
		m.putAll(asRow);
		return m;
