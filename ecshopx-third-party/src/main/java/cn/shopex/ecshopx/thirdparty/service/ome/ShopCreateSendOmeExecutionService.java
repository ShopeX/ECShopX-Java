...
@Service
public class ShopCreateSendOmeExecutionService {

	private static final Logger log = LoggerFactory.getLogger(ShopCreateSendOmeExecutionService.class);
	...
	private final OmeOpenApiShopAddPort omeOpenApiShopAddPort;

	public ShopCreateSendOmeExecutionService(
			ShopexErpOpenApiSettingReadPort shopexErpOpenApiSettingReadPort,
			OmeDistributorShopStructPort omeDistributorShopStructPort,
			OmeOpenApiShopAddPort omeOpenApiShopAddPort) {
		...
		this.omeOpenApiShopAddPort = omeOpenApiShopAddPort;
	}
	...
	public void executeFromDispatchPayload(Map<String, Object> payload) {
		try {
			...
			log.debug("shop create OME dispatch failed: {}", e.getMessage(), e);
		}
	}
	...
	private void run(Map<String, Object> payload) {
		...
		if (!(rawEntities instanceof Map<?, ?> rawMap)) {
			log.debug("shop create OME skipped: missing entities map");
			return;
		}
		...
		if (companyId == null || companyId <= 0L || distributorId == null || distributorId <= 0L) {
			log.debug("shop create OME skipped: invalid company_id or distributor_id");
			return;
		}
		var settingOpt = shopexErpOpenApiSettingReadPort.loadParsed(companyId);
		if (settingOpt.isEmpty()) {
			log.debug("shop create OME skipped: no ERP setting companyId={}", companyId);
			return;
		}
		Map<String, Object> setting = settingOpt.get();
		if (!Boolean.TRUE.equals(setting.get("is_openapi_open"))) {
			log.debug("shop create OME skipped: openapi not enabled companyId={}", companyId);
			return;
		}
		var structOpt = omeDistributorShopStructPort.tryBuildShopAdd(companyId, distributorId);
		if (structOpt.isEmpty()) {
			log.debug("shop create OME skipped: empty shop struct companyId={} distributorId={}", companyId, distributorId);
			return;
		}
		Map<String, Object> shopStruct = structOpt.get();
		Map<String, Object> response = omeOpenApiShopAddPort.callShopAdd(companyId, shopStruct);
		log.debug("shop create OME shop.add companyId={} distributorId={} rsp={}", companyId, distributorId, response);
	}
