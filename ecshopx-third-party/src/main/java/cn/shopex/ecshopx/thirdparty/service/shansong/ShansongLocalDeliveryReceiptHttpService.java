...
@Service("shansongLocalDeliveryReceiptHttp")
@ConditionalOnProperty(prefix = "ecshopx.thirdparty.shansong", name = "http-enabled", havingValue = "true")
public class ShansongLocalDeliveryReceiptHttpService implements ShansongLocalDeliveryReceiptPort {
	...
	public ShansongLocalDeliveryReceiptHttpService(
	...
	@Override
	public void orderPlaceApi(long companyId, String issOrderNo) {
		if (!StringUtils.hasText(issOrderNo)) {
			throw new ResourceException("闪送发单缺少订单号");
		}
		...
		String url = host + "/openapi/merchants/v5/orderPlace";
		try {
			...
			if (!resp.getStatusCode().is2xxSuccessful()) {
				throw new ResourceException("闪送发单请求失败");
			}
			...
			if (status != HTTP_OK) {
				String msg = root.path("msg").asText("闪送发单失败");
				throw new ResourceException(msg);
			}
		} catch (ResourceException e) {
			throw e;
		} catch (Exception e) {
			throw new ResourceException("闪送发单调用异常: " + e.getMessage());
		}
	}
