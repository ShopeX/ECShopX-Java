...
class DistributionEditPushMarketingCenterProcessorTest {

	@Mock
	private MarketingCenterOpenApiSignedFormClient marketingCenterOpenApiSignedFormClient;

	private DistributionEditPushMarketingCenterProcessor processor;

	@BeforeEach
	void setUp() {
		processor = new DistributionEditPushMarketingCenterProcessor(marketingCenterOpenApiSignedFormClient);
	}

	@Test
	void handle_whenCompanyAndDistributorPresent_invokesBasicsDistributionEditProccessWithExpectedParams() {
		...
		verify(marketingCenterOpenApiSignedFormClient).basicsDistributionEditProccess(eq(77L), argThat(params -> {
			if (params == null) {
				return false;
			}
	...
	void handle_acceptsStringCompanyIdInEntities() {
		...
		verify(marketingCenterOpenApiSignedFormClient).basicsDistributionEditProccess(eq(55L), argThat(params -> {
			if (params == null) {
				return false;
			}
