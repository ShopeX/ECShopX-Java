...
class DmMessageNotifyWebhookServiceCardTemplateDeleteTopicTest {
	...
	@Test
	@DisplayName("card + cardTemplateDelete: normalizes topic before kaquan port; original map unchanged")
	void handle_whenTopicIsCardAndEventIsCardTemplateDelete_normalizesTopicAndInvokesKaquanPort() {
		DmMessageNotifyWebhookService svc = new DmMessageNotifyWebhookService(kaquanPort);
		Map<String, Object> merged = new LinkedHashMap<>();
		merged.put("topic", "card");
		merged.put("event", "cardTemplateDelete");
		...
		assertThat(captor.getValue()).containsEntry("topic", "sync_card_template_delete");
		assertThat(merged).containsEntry("topic", "card");
	}

	@Test
	@DisplayName("SYNC_CARD_TEMPLATE_DELETE casing: still invokes port with canonical topic")
	void handle_whenTopicIsMixedCaseDelete_passesLowercaseCanonicalTopic() {
		DmMessageNotifyWebhookService svc = new DmMessageNotifyWebhookService(kaquanPort);
		Map<String, Object> merged = new LinkedHashMap<>();
